...
Public Class mat_insp
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            da = New SqlDataAdapter("select distinct PO_RCD_MAT.CRR_NO from PO_RCD_MAT JOIN ORDER_DETAILS ON PO_RCD_MAT.PO_NO=ORDER_DETAILS.SO_NO  where PO_RCD_MAT.INSP_EMP is null AND (ORDER_DETAILS.PO_TYPE='STORE MATERIAL' OR ORDER_DETAILS.PO_TYPE='STORE MATERIAL(IMP)')  ORDER BY PO_RCD_MAT.CRR_NO", conn)
    ...
    Protected Sub Button26_Click(sender As Object, e As EventArgs) Handles Button26.Click
        ...
        Next
        'mat value
        Dim orderQuantity, total_value, unit_value As Decimal
        ...
        If dr.HasRows Then
            dr.Read()
            orderQuantity = dr.Item("MAT_QTY")
            'rqty = dr.Item("MAT_QTY_RCVD")
            dr.Close()
        End If
        conn.Close()

        Dim RCVD_QTY_TILL_DATE As Decimal = 0
        conn.Open()
        mc1.CommandText = "select (SUM(PO_RCD_MAT.MAT_RCD_QTY) - SUM(PO_RCD_MAT.MAT_REJ_QTY) - SUM(PO_RCD_MAT.MAT_EXCE)) AS MAT_RCD_QTY_TILL_DATE from PO_RCD_MAT join MATERIAL on PO_RCD_MAT .MAT_CODE =MATERIAL .MAT_CODE  where PO_RCD_MAT.PO_NO ='" & po_no_TextBox.Text & "' AND PO_RCD_MAT .MAT_SLNO =" & MATCODE_DropDownList.SelectedValue & " AND INSP_DATE IS NOT NULL"
        mc1.Connection = conn
        dr = mc1.ExecuteReader
        If dr.HasRows Then
            dr.Read()
            If IsDBNull(dr.Item("MAT_RCD_QTY_TILL_DATE")) Then
                RCVD_QTY_TILL_DATE = 0.00
            Else
                RCVD_QTY_TILL_DATE = (dr.Item("MAT_RCD_QTY_TILL_DATE"))
            End If
        ...
        If (orderQuantity - (RCVD_QTY_TILL_DATE) - (CRR_RCVD_QTY - CDec(GARN_REJQTYTextBox.Text)) > 0) Then
            bal_qty = orderQuantity - (RCVD_QTY_TILL_DATE) - (CRR_RCVD_QTY - CDec(GARN_REJQTYTextBox.Text))
        Else
            bal_qty = 0
        End If
    ...
    Protected Sub Button28_Click(sender As Object, e As EventArgs) Handles Button28.Click
        Using conn_trans
            ...
            Try
                ...
                For I = 0 To GridView1.Rows.Count - 1
                    'conn.Open()
                    mycommand = New SqlCommand("update PO_RCD_MAT set MAT_REJ_QTY=" & GridView1.Rows(I).Cells(6).Text & " , INSP_EMP='" & Session("userName") & "' , INSP_DATE='" & Today.Year & "-" & Today.Month & "-" & Today.Day & "',MAT_BAL_QTY=" & CDec(GridView1.Rows(I).Cells(9).Text) & ",INSP_NOTE='" & GridView1.Rows(I).Cells(10).Text & "' where MAT_SLNO='" & GridView1.Rows(I).Cells(1).Text & "' and CRR_NO='" & CRR_DropDownList.SelectedValue & "' and po_no='" & po_no_TextBox.Text & "'", conn_trans, myTrans)
                ...
                conn.Open()
                Dim ds5 As New DataSet
                da = New SqlDataAdapter("select distinct PO_RCD_MAT.CRR_NO from PO_RCD_MAT WITH(NOLOCK) JOIN ORDER_DETAILS ON PO_RCD_MAT.PO_NO=ORDER_DETAILS.SO_NO  where PO_RCD_MAT.INSP_EMP is null AND ORDER_DETAILS.PO_TYPE='STORE MATERIAL'  ORDER BY PO_RCD_MAT.CRR_NO", conn)
                da.Fill(ds5, "PO_RCD_MAT")
                CRR_DropDownList.DataSource = ds5.Tables("PO_RCD_MAT")
                CRR_DropDownList.DataValueField = "CRR_NO"
                CRR_DropDownList.DataBind()
    ...
    Protected Sub Button27_Click(sender As Object, e As EventArgs) Handles Button27.Click
        ...
        conn.Open()
        Dim ds5 As New DataSet
        da = New SqlDataAdapter("select distinct PO_RCD_MAT.CRR_NO from  PO_RCD_MAT JOIN ORDER_DETAILS ON PO_RCD_MAT.PO_NO=ORDER_DETAILS.SO_NO  where PO_RCD_MAT.INSP_EMP is null AND ORDER_DETAILS.PO_TYPE='STORE MATERIAL'  ORDER BY PO_RCD_MAT.CRR_NO", conn)
        da.Fill(ds5, "PO_RCD_MAT")
        CRR_DropDownList.DataSource = ds5.Tables("PO_RCD_MAT")
        CRR_DropDownList.DataValueField = "CRR_NO"
        CRR_DropDownList.DataBind()

...
Public Class rm_insp
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            po_no_TextBox.Attributes.Add("readonly", "readonly")
            AU_GRANTextBox.Attributes.Add("readonly", "readonly")
            TextBox184.Attributes.Add("readonly", "readonly")
            RCVDQTY_TextBox.Attributes.Add("readonly", "readonly")
        End If
    ...
    Protected Sub Button26_Click(sender As Object, e As EventArgs) Handles Button26.Click
        ...
        If dr.HasRows Then
            ...
            If IsDBNull(dr.Item("TOLERANCE")) Then
                tolerance = 0
            Else
                tolerance = dr.Item("TOLERANCE") / 100
            End If
        ...
        Dim orderQty, previousRCDQty, PREVIOUS_REJ_QTY, PREVIOUS_EXCESS_QTY, total_value, unit_value As Decimal
        ...
        mc1.CommandText = "select * from PO_ORD_MAT join MATERIAL on PO_ORD_MAT .mat_code=MATERIAL.MAT_CODE where PO_ORD_MAT .mat_slno=" & MATCODE_DropDownList.SelectedValue & " and PO_ORD_MAT .po_no='" & po_no_TextBox.Text & "'"
        mc1.Connection = conn
        ...
        If dr.HasRows Then
            dr.Read()
            orderQty = dr.Item("MAT_QTY")
            'rqty = dr.Item("MAT_QTY_RCVD")
            dr.Close()
        End If
        conn.Close()
        Dim EFFECTIVE_CRR_RCVD_QTY, CURRENT_CRR_EXCESS_QTY, CURRENT_CRR_RCD_QTY As New Decimal(0)
        conn.Open()

        If (transporter = "N/A") Then
            If (tolerance <> 0) Then
                ''TOLERANCE
                mc1.CommandText = "select ((CASE WHEN (MAT_CHALAN_QTY-MAT_RCD_QTY)<(MAT_CHALAN_QTY*" & tolerance & ") THEN MAT_CHALAN_QTY ELSE MAT_RCD_QTY END) - PO_RCD_MAT.MAT_EXCE) as EFFECTIVE_MAT_RCD_QTY,PO_RCD_MAT.MAT_RCD_QTY,PO_RCD_MAT.TRANS_WO_NO,PO_RCD_MAT.MAT_EXCE ,MATERIAL .MAT_AU,MATERIAL.MAT_NAME from PO_RCD_MAT join MATERIAL on PO_RCD_MAT .MAT_CODE =MATERIAL .MAT_CODE  where PO_RCD_MAT .MAT_SLNO =" & MATCODE_DropDownList.SelectedValue & " and PO_RCD_MAT .CRR_NO='" & CRR_DropDownList.SelectedValue & "'"

            Else
                mc1.CommandText = "select (MAT_CHALAN_QTY - PO_RCD_MAT.MAT_EXCE) as EFFECTIVE_MAT_RCD_QTY,PO_RCD_MAT.MAT_RCD_QTY,PO_RCD_MAT.TRANS_WO_NO,PO_RCD_MAT.MAT_EXCE ,MATERIAL .MAT_AU,MATERIAL.MAT_NAME from PO_RCD_MAT join MATERIAL on PO_RCD_MAT .MAT_CODE =MATERIAL .MAT_CODE  where PO_RCD_MAT .MAT_SLNO =" & MATCODE_DropDownList.SelectedValue & " and PO_RCD_MAT .CRR_NO='" & CRR_DropDownList.SelectedValue & "'"
            End If
        ...
        If dr.HasRows Then
            dr.Read()
            EFFECTIVE_CRR_RCVD_QTY = (dr.Item("EFFECTIVE_MAT_RCD_QTY"))
            transporter = (dr.Item("TRANS_WO_NO"))
            CURRENT_CRR_EXCESS_QTY = (dr.Item("MAT_EXCE"))
            CURRENT_CRR_RCD_QTY = (dr.Item("MAT_RCD_QTY"))
            dr.Close()
        Else
            conn.Close()
        End If
        ...
        If (transporter = "N/A") Then
            If (tolerance <> 0) Then
                ''TOLERANCE
                queryString = "SELECT sum((CASE WHEN (MAT_CHALAN_QTY-MAT_RCD_QTY)<(MAT_CHALAN_QTY*" & tolerance & ") THEN MAT_CHALAN_QTY ELSE MAT_RCD_QTY END))-sum(MAT_REJ_QTY)-sum(MAT_EXCE) as MAT_QTY_RCVD, sum(MAT_REJ_QTY) as PREVIOUS_REJ_QTY, sum(MAT_EXCE) as PREVIOUS_EXCESS_QTY FROM PO_RCD_MAT where PO_NO='" & po_no_TextBox.Text & "' and MAT_SLNO=" & MATCODE_DropDownList.SelectedValue & " and INSP_EMP is not null"

            Else
                queryString = "SELECT sum(MAT_RCD_QTY)-sum(MAT_REJ_QTY)-sum(MAT_EXCE) as MAT_QTY_RCVD, sum(MAT_REJ_QTY) as PREVIOUS_REJ_QTY, sum(MAT_EXCE) as PREVIOUS_EXCESS_QTY FROM PO_RCD_MAT where PO_NO='" & po_no_TextBox.Text & "' and MAT_SLNO=" & MATCODE_DropDownList.SelectedValue & " and INSP_EMP is not null"
            End If

        Else
            queryString = "select (sum(MAT_CHALAN_QTY)-sum(MAT_REJ_QTY)-sum(MAT_EXCE)) AS MAT_QTY_RCVD, sum(MAT_REJ_QTY) as PREVIOUS_REJ_QTY, sum(MAT_EXCE) as PREVIOUS_EXCESS_QTY from PO_RCD_MAT where PO_NO='" & po_no_TextBox.Text & "' and MAT_SLNO=" & MATCODE_DropDownList.SelectedValue & " and INSP_EMP is not null"
        End If
        ...
        If dr.HasRows Then
            ...
            If IsDBNull(dr.Item("MAT_QTY_RCVD")) Then
                previousRCDQty = 0
            Else
                previousRCDQty = dr.Item("MAT_QTY_RCVD")
            End If
        ...
        bal_qty = orderQty - previousRCDQty - EFFECTIVE_CRR_RCVD_QTY + CDec(GARN_REJQTYTextBox.Text)
        If bal_qty > 0 Then
            exces_qty = 0.0
        Else
            exces_qty = (orderQty - previousRCDQty - EFFECTIVE_CRR_RCVD_QTY + CURRENT_CRR_EXCESS_QTY)
        End If
        Dim dt As DataTable = DirectCast(ViewState("mat1"), DataTable)
        dt.Rows.Add(po_no_TextBox.Text, MATCODE_DropDownList.SelectedValue, mat_id, mat_name, AU_GRANTextBox.Text, RCVDQTY_TextBox.Text, GARN_REJQTYTextBox.Text, (CDec(RCVDQTY_TextBox.Text) - CDec(GARN_REJQTYTextBox.Text)) - exces_qty, exces_qty, bal_qty, NOTE_TextBox.Text)
        ViewState("mat1") = dt
        Me.BINDGRID()
    End Sub
    ...
    Protected Sub Button28_Click(sender As Object, e As EventArgs) Handles Button28.Click
        Using conn_trans
            ...
            Try
                ...
                conn.Open()
                Dim ds5 As New DataSet
                da = New SqlDataAdapter("select distinct PO_RCD_MAT.CRR_NO from PO_RCD_MAT WITH(NOLOCK) JOIN ORDER_DETAILS ON PO_RCD_MAT.PO_NO=ORDER_DETAILS.SO_NO  where PO_RCD_MAT.INSP_EMP is null AND (ORDER_DETAILS.PO_TYPE='RAW MATERIAL' or ORDER_DETAILS .PO_TYPE ='RAW MATERIAL(IMP)') ORDER BY PO_RCD_MAT.CRR_NO", conn)
                da.Fill(ds5, "PO_RCD_MAT")
                CRR_DropDownList.DataSource = ds5.Tables("PO_RCD_MAT")
                CRR_DropDownList.DataValueField = "CRR_NO"
                CRR_DropDownList.DataBind()
                conn.Close()
                CRR_DropDownList.Items.Add("Select")
                CRR_DropDownList.SelectedValue = "Select"
                CRR_DropDownList.Enabled = True
    ...
    Protected Sub MATCODE_DropDownList_SelectedIndexChanged(sender As Object, e As EventArgs) Handles MATCODE_DropDownList.SelectedIndexChanged
        ...
        conn.Open()
        Dim mc1 As New SqlCommand
        mc1.CommandText = "select PO_RCD_MAT.MAT_RCD_QTY,PO_RCD_MAT.MAT_EXCE ,MATERIAL .MAT_AU,MATERIAL.MAT_NAME from PO_RCD_MAT join MATERIAL on PO_RCD_MAT .MAT_CODE =MATERIAL .MAT_CODE  where PO_RCD_MAT .MAT_SLNO =" & MATCODE_DropDownList.SelectedValue & " and PO_RCD_MAT .CRR_NO='" & CRR_DropDownList.SelectedValue & "' "
        mc1.Connection = conn
        dr = mc1.ExecuteReader
        If dr.HasRows Then
            dr.Read()
            RCVDQTY_TextBox.Text = (dr.Item("MAT_RCD_QTY"))
            AU_GRANTextBox.Text = dr.Item("MAT_AU")
            TextBox184.Text = dr.Item("MAT_NAME")
            dr.Close()
        Else
            conn.Close()
        End If

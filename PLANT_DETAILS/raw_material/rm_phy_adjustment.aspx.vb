...
Public Class rm_phy_adjustment
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            conn.Open()
            dt.Clear()
            da = New SqlDataAdapter("select distinct(MAT_CODE +' , '+ MAT_NAME) as mat_detail from MATERIAL where MAT_CODE like '100%' ", conn)
            da.Fill(dt)
            conn.Close()
    ...
    Private Sub MAT_DETAILS_ADJUST(ITEM_CODE As String, ADJ_DATE As Date, ADJ_QTY As Decimal, name_user As String)
        ...
        conn.Open()
        ds.Clear()
        da = New SqlDataAdapter("SELECT distinct(ISSUE_NO) FROM MAT_DETAILS WHERE ISSUE_NO LIKE '%ADJ%' and FISCAL_YEAR=" & STR1, conn)
        count = da.Fill(dt)
        conn.Close()

        If count = 0 Then
            Adj_no_TextBox.Text = "ADJ" & STR1 & "000001"
        Else
            str = count + 1
            ...
            ElseIf str.Length = 5 Then
                str = "0" & (count + 1)
            End If
            Adj_no_TextBox.Text = "ADJ" & STR1 & str
        End If
        ...
        If (ADJ_QTY > 0) Then
            ...
            cmd1.Parameters.AddWithValue("@RQD_QTY", (-1) * ADJ_QTY)
            cmd1.Parameters.AddWithValue("@ISSUE_QTY", (-1) * ADJ_QTY)
        ...
        conn.Open()
        Dim mc1 As New SqlCommand
        mc1.CommandText = "select * from MATERIAL WITH(NOLOCK) where MAT_CODE='" & DropDownList1.Text.Substring(0, (DropDownList1.Text.IndexOf(",") - 1)).Trim & "' "
        mc1.Connection = conn
        dr = mc1.ExecuteReader
        If dr.HasRows Then
            dr.Read()
        ...
        ''Save ledger
        If (flag = True) Then
            save_ledger("Physical Adjustment", Adj_no_TextBox.Text, "", "", ac_issue, "Dr", ADJ_QTY * CDec(TextBox2.Text), "RM_Physical_Adj", "", 1, "")
            save_ledger("Physical Adjustment", Adj_no_TextBox.Text, "", "", ac_con, "Cr", ADJ_QTY * CDec(TextBox2.Text), "RM_Physical_Adj", "", 2, "")
        Else
            save_ledger("Physical Adjustment", Adj_no_TextBox.Text, "", "", ac_con, "Dr", ADJ_QTY * CDec(TextBox2.Text), "RM_Physical_Adj", "", 1, "")
            save_ledger("Physical Adjustment", Adj_no_TextBox.Text, "", "", ac_issue, "Cr", ADJ_QTY * CDec(TextBox2.Text), "RM_Physical_Adj", "", 2, "")
        End If
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        '& " where PROD_CONTROL .PROD_DATE = '" & dpr_date.Year & "-" & dpr_date.Month & "-" & dpr_date.Day & "' and ITEM_I_SO ='" & Adj_no_TextBox.Text & "'" _
        '& " group by PROD_CONTROL .ITEM_CODE,F_ITEM .ITEM_NAME,qual_group.qual_name,F_ITEM .MAT_AU"

        Dim PO_QUARY As String = "select distinct MAT_DETAILS .MAT_CODE, material .MAT_NAME ,material .mat_au , '" & dpr_date & "'  AS P_DATE , '" & dpr_date & "'  AS P_DATE_TO ,
            '" & DropDownList5.SelectedValue & "' AS R_TYPE,  (case when MAT_DETAILS.MAT_QTY =0 then ISSUE_QTY else MAT_DETAILS.MAT_QTY end) as INV_QTY
            from MAT_DETAILS join MATERIAL on MAT_DETAILS .MAT_CODE =MATERIAL.MAT_CODE where MAT_DETAILS .LINE_DATE = '" & dpr_date.Year & "-" & dpr_date.Month & "-" & dpr_date.Day & "'
            and issue_no ='" & Adj_no_TextBox.Text & "'"

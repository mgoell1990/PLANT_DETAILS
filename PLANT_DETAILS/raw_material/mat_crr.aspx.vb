...
Public Class mat_crr
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            type_DropDown.Items.Add("RAW MATERIAL")
            type_DropDown.Items.Add("RAW MATERIAL(IMP)")
            ...
            bl_noTextBox.Attributes.Add("readonly", "readonly")
    ...
    Protected Sub mat_sl_noDropDownList_SelectedIndexChanged(sender As Object, e As EventArgs) Handles mat_sl_noDropDownList.SelectedIndexChanged
        ...
        Dim mc1 As New SqlCommand
        conn.Open()
        mc1.CommandText = "SELECT PO_ORD_MAT .MAT_CODE ,PO_ORD_MAT .MAT_NAME ,MATERIAL .MAT_AU  FROM PO_ORD_MAT JOIN MATERIAL  ON PO_ORD_MAT .MAT_CODE  =MATERIAL .MAT_CODE WHERE PO_ORD_MAT .MAT_SLNO =" & mat_sl_noDropDownList.Text.Substring(0, mat_sl_noDropDownList.Text.IndexOf(",") - 1) & "and po_ord_mat.po_no='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "'"
        mc1.Connection = conn
        dr = mc1.ExecuteReader
        If dr.HasRows Then
            dr.Read()
    ...
    Protected Sub crr_add_Button_Click(sender As Object, e As EventArgs) Handles crr_add_Button.Click
        ...
        conn.Close()
        ''be search
        If po_type = "RAW MATERIAL(IMP)" Then
            conn.Open()
        ...
        If be_TextBox.Text <> "N/A" Then
            mc1.CommandText = "select (sum(MAT_RCD_QTY) - sum(MAT_REJ_QTY) - sum(MAT_EXCE)) As MAT_QTY_RCVD, (sum(MAT_CHALAN_QTY) - sum(MAT_REJ_QTY) - sum(MAT_EXCE)) as MAT_QTY_CONSIDERED_FOR_PAYMENT from PO_RCD_MAT where po_no='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "' and MAT_SLNO='" & mat_sl_noDropDownList.Text.Substring(0, mat_sl_noDropDownList.Text.IndexOf(",") - 1) & "'"
        Else
        ...
        Dim w_tolerance, PRICE, w_unit_price, W_discount, DISCOUNT_VALUE, qty_considered_current_crr As New Decimal(0.00)
        ...
        If trans_DropDownList.SelectedValue <> "N/A" Then
            ...
            If WO_Qty < Work_completed + CDec(chalan_qty_TextBox.Text) Then
                chalan_qty_TextBox.Focus()
                Label467.Visible = True
        ...
        Dim CHA_WO, CHA_SLNO As New String("")
        ...
        If be_TextBox.Text <> "N/A" Then
            ...
            Work_completed = 0
            conn.Open()
            mc1.CommandText = "select sum(W_QTY) As WO_Qty, sum(W_COMPLITED) As Work_completed from WO_ORDER where PO_NO='" & CHA_WO & "' and W_SLNO='" & CHA_SLNO & "' group by PO_NO,W_SLNO"
            mc1.Connection = conn
            dr = mc1.ExecuteReader
            If dr.HasRows Then
                dr.Read()
            ...
            If WO_Qty < Work_completed Then
                chalan_qty_TextBox.Focus()
                Label467.Visible = True
                Label467.Text = "CHA Work Order do not have balance"
                Return
            ElseIf WO_Qty < Work_completed + CDec(chalan_qty_TextBox.Text) Then
                chalan_qty_TextBox.Focus()
                Label467.Visible = True
                Label467.Text = "Chalan Qty is Higher Than CHA Work Order Balance Qty which is. " & WO_Qty - Work_completed
                Return
            Else
                Label467.Text = ""
                Label467.Visible = False
        ...
        PRICE = PRICE * CDec(rcv_qty_TextBox.Text)
        ...
        Else
            qty_considered_current_crr = CDec(net_rcd_qty.Text)
        End If
        ...
        If trans_DropDownList.SelectedValue <> "N/A" Then
            ...
            End If
        Else
            If (qty - MAT_QTY_CONSIDERED_FOR_PAYMENT) < qty_considered_current_crr Then
                exqty = qty_considered_current_crr - (qty - MAT_QTY_CONSIDERED_FOR_PAYMENT)
            Else
                exqty = 0.0
            End If
        End If
            ...
            Try
                ...
                Else
                    ...
                    For I = 0 To crr_gridview.Rows.Count - 1
                        ...
                        Dim MAT_CHALAN_QTY As String = CDec(crr_gridview.Rows(I).Cells(12).Text) - ((CDec(crr_gridview.Rows(I).Cells(17).Text) * CDec(crr_gridview.Rows(I).Cells(18).Text)) / 1000)
                        Dim MAT_RCD_QTY As String = CDec(crr_gridview.Rows(I).Cells(13).Text) - ((CDec(crr_gridview.Rows(I).Cells(17).Text) * CDec(crr_gridview.Rows(I).Cells(18).Text)) / 1000)
                        'Dim MAT_RCD_QTY As String = CDec(crr_gridview.Rows(I).Cells(13).Text)
                        ...
                        If trans_DropDownList.Text = "N/A" Then
                            ...
                            'RCVD_QTY_FOR = CDec(crr_gridview.Rows(I).Cells(13).Text) - CDec(crr_gridview.Rows(I).Cells(15).Text) - ((CDec(crr_gridview.Rows(I).Cells(17).Text) * CDec(crr_gridview.Rows(I).Cells(18).Text)) / 1000)
                    ...
                    ''UPDATE TRANSPORTER
                    If trans_DropDownList.SelectedValue <> "N/A" Then
                        Dim C_QTY, R_QTY, TOTAL_QTY, N_BAG, B_WEIGHT, FAULT_PRICE, WEIGHT_TO_BE_CONSIDERED As Decimal
                        C_QTY = 0.0
                        R_QTY = 0.0
                        N_BAG = 0.0
                        FAULT_PRICE = 0.0
                        ...
                        For count = 0 To crr_gridview.Rows.Count - 1
                            ...
                            chln_qty_trans = chln_qty_trans + C_QTY
                            rcvd_qty_transporter = rcvd_qty_transporter + R_QTY
 ...
 " ORDER BY PO_RCD_MAT.MAT_SLNO"
    ...
    Protected Sub DropDownList1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DropDownList1.SelectedIndexChanged
        ...
        If (DropDownList1.SelectedValue <> "Select") Then
            conn.Open()
            da = New SqlDataAdapter("select distinct CRR_NO from PO_RCD_MAT where CRR_NO like 'RCRR%' and crr_date between '" & 2000 + CInt(Left(DropDownList1.SelectedValue, 2)) & "-04-01' and '" & 2000 + CInt(Right(DropDownList1.SelectedValue, 2)) & "-03-31' ORDER BY CRR_NO", conn)

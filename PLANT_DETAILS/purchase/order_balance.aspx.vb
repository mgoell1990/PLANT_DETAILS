...
Public Class order_balance
    ...
    Protected Sub Button30_Click(sender As Object, e As EventArgs) Handles Button30.Click
        ...
        Try
            ...
            Using wb As XLWorkbook = New XLWorkbook()
                wb.Worksheets.Add(dt, "Order Balance")
                Response.Clear()
                Response.Buffer = True
                Response.Charset = ""
                Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    ...
    Protected Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        Try
            Panel14.Visible = True
            If (TextBox3.Text.Substring(0, 1) = "P") Then
                conn.Open()
                dt.Clear()

                da = New SqlDataAdapter("DECLARE @tolerance DECIMAL(6,2) = 0;
			set @tolerance = (select TOLERANCE from ORDER_DETAILS where SO_NO='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "')
			DECLARE @TT TABLE(PO_NO VARCHAR(60),W_SLNO VARCHAR(30),W_NAME VARCHAR(150),W_AU VARCHAR(30),UNIT_PRICE DECIMAL(16,2),W_START_DATE VARCHAR(30),W_END_DATE VARCHAR(30),W_QTY DECIMAL(16,3))
			INSERT INTO @TT
			select PO_NO,MAT_SLNO as W_SLNO,PO_ORD_MAT.MAT_NAME as W_NAME,MAX(MATERIAL.MAT_AU) AS W_AU,SUM(MAT_UNIT_RATE) AS UNIT_PRICE,MIN(AMD_DATE) AS W_START_DATE,MAX(MAT_DELIVERY) AS W_END_DATE,SUM(MAT_QTY) AS W_QTY from PO_ORD_MAT join MATERIAL on PO_ORD_MAT.MAT_CODE=MATERIAL.MAT_CODE where PO_NO ='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "' GROUP BY PO_NO,MAT_SLNO,PO_ORD_MAT.MAT_NAME ORDER BY MAT_SLNO

			DECLARE @TT1 TABLE(PO_NO VARCHAR(30),W_SLNO VARCHAR(30),W_COMPLETED VARCHAR(30))
			INSERT INTO @TT1
			
			select PO_NO,MAT_SLNO as W_SLNO,sum((case when TRANS_WO_NO = 'N/A' THEN (case when @tolerance=0 THEN (MAT_RCD_QTY- MAT_REJ_QTY-MAT_EXCE) else (case when (MAT_CHALAN_QTY-MAT_RCD_QTY)<=(MAT_CHALAN_QTY*0.005) THEN (MAT_CHALAN_QTY- MAT_REJ_QTY-MAT_EXCE) else (MAT_RCD_QTY- MAT_REJ_QTY-MAT_EXCE) end) end) else MAT_CHALAN_QTY end)) AS W_COMPLETED from PO_RCD_MAT where PO_NO='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "' and GARN_NO<>'PENDING' group by PO_NO,MAT_SLNO
			...
			SELECT T1.PO_NO,T1.W_SLNO,'' as vocabNo,T1.W_NAME,T1.W_AU,t1.UNIT_PRICE,T1.W_START_DATE,T1.W_END_DATE,T1.W_QTY,ISNULL(T2.W_COMPLETED, 0.000) As W_COMPLETED FROM @TT T1 LEFT JOIN @TT1 T2 ON T1.PO_NO=T2.PO_NO AND T1.W_SLNO=T2.W_SLNO order by t1.PO_NO,convert(int,T1.W_SLNO)", conn)
                ...
                Dim mc1 As New SqlCommand
                conn.Open()
                mc1.CommandText = "select supl.supl_name from supl join ORDER_DETAILS on ORDER_DETAILS.PARTY_CODE=supl.supl_id where ORDER_DETAILS.SO_NO  = '" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "'"
                mc1.Connection = conn
                dr = mc1.ExecuteReader
                If dr.HasRows Then
                    dr.Read()
                ...
                End If
                conn.Close()
            ElseIf (TextBox3.Text.Substring(0, 1) = "W") Then
                conn.Open()
                dt.Clear()

                da = New SqlDataAdapter("DECLARE @TT TABLE(PO_NO VARCHAR(60),W_SLNO VARCHAR(30),W_NAME VARCHAR(500),W_AU VARCHAR(30),UNIT_PRICE DECIMAL(16,2),W_START_DATE VARCHAR(30),W_END_DATE VARCHAR(30),W_QTY DECIMAL(16,3))
			INSERT INTO @TT
			select PO_NO,W_SLNO,W_NAME,MAX(W_AU) AS W_AU,SUM(W_UNIT_PRICE) AS UNIT_PRICE,MIN(W_START_DATE) AS W_START_DATE,MAX(W_END_DATE) AS W_END_DATE,SUM(W_QTY) AS W_QTY from WO_ORDER where WO_ORDER.PO_NO ='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "' GROUP BY PO_NO,W_SLNO,W_NAME ORDER BY W_SLNO

			DECLARE @TT1 TABLE(PO_NO VARCHAR(30),W_SLNO VARCHAR(30),W_COMPLETED VARCHAR(30))
			INSERT INTO @TT1
			select PO_NO,WO_SLNO,SUM(work_qty) AS W_COMPLETED from mb_book where PO_NO='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "' GROUP BY po_no,wo_slno
			...
			SELECT T1.PO_NO,T1.W_SLNO,'' as vocabNo,T1.W_NAME,T1.W_AU,t1.UNIT_PRICE,T1.W_START_DATE,T1.W_END_DATE,T1.W_QTY,ISNULL(T2.W_COMPLETED, 0.000) As W_COMPLETED FROM @TT T1 LEFT JOIN @TT1 T2 ON T1.PO_NO=T2.PO_NO AND T1.W_SLNO=T2.W_SLNO", conn)
                ...
                Dim mc1 As New SqlCommand
                conn.Open()
                mc1.CommandText = "select supl.supl_name from supl join wo_order on wo_order.supl_id=supl.supl_id where wo_order.po_no  = '" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "'"
                mc1.Connection = conn
                dr = mc1.ExecuteReader
                If dr.HasRows Then
                    dr.Read()
                ...
                End If
                conn.Close()
            ElseIf (TextBox3.Text.Substring(0, 1) = "R") Then
                Dim PO_TYPE, order_amt, prov_value As New String("")
                Dim mc1 As New SqlCommand
                conn.Open()
                mc1.CommandText = "select * from ORDER_DETAILS where SO_NO = '" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "'"
                mc1.Connection = conn
                dr = mc1.ExecuteReader
                If dr.HasRows Then
                    dr.Read()
                ...
                If (PO_TYPE = "STORE MATERIAL" Or PO_TYPE = "Others") Then
                    ...
                    da = New SqlDataAdapter("DECLARE @TT TABLE(PO_NO VARCHAR(60),W_SLNO VARCHAR(30),vocabNo VARCHAR(30),W_NAME VARCHAR(150),W_AU VARCHAR(30),UNIT_PRICE DECIMAL(16,2),W_START_DATE VARCHAR(30),W_END_DATE VARCHAR(30),W_QTY DECIMAL(16,3),W_COMPLETED DECIMAL(16,3))
			INSERT INTO @TT
			select PO_NO,MAT_SLNO,'' as vocabNo,MAT_NAME,MAX(MAT_AU) AS W_AU,SUM(MAT_UNIT_RATE) AS UNIT_PRICE,MIN(AMD_DATE) AS W_START_DATE,MAX(MAT_DELIVERY) AS W_END_DATE,SUM(MAT_QTY) AS W_QTY,SUM(MAT_QTY_RCVD) AS W_COMPLETED from PO_ORD_MAT where PO_ORD_MAT.PO_NO ='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "' GROUP BY PO_NO,MAT_SLNO,MAT_NAME ORDER BY MAT_SLNO
			select  * from @TT", conn)
			...
			SELECT T1.PO_NO,T1.W_SLNO,'' as vocabNo,T1.W_NAME,T1.W_AU,t1.UNIT_PRICE,T1.W_START_DATE,T1.W_END_DATE,T1.W_QTY,ISNULL(T2.W_COMPLETED, 0.000) As W_COMPLETED FROM @TT T1 LEFT JOIN @TT1 T2 ON T1.PO_NO=T2.PO_NO AND T1.W_SLNO=T2.W_SLNO", conn)
                ...
                'conn.Open()
                'mc1.CommandText = "select supl.supl_name from supl join RATE_CONTRACT on wo_order.supl_id=supl.supl_id where wo_order.po_no  = '" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "'"
                'mc1.Connection = conn
                'dr = mc1.ExecuteReader
                'If dr.HasRows Then
                '    dr.Read()
                ...
                Dim mc2 As New SqlCommand
                If (PO_TYPE = "STORE MATERIAL" Or PO_TYPE = "Others") Then
                    mc2.CommandText = "select T1.PO_NO,t1.ORD_AMOUNT,t2.prov_value,t3.SUPL_NAME from RATE_CONTRACT T1 INNER JOIN (select SUPL_ID,PO_NO,sum(PROV_VALUE) as prov_value from PO_RCD_MAT where PO_NO='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "' group by SUPL_ID,PO_NO) as T2
					ON T1.PO_NO = T2.PO_NO join SUPL T3 on t2.SUPL_ID=T3.SUPL_ID"

                ElseIf (PO_TYPE = "Maint Work" Or PO_TYPE = "Works Contract") Then

                    mc2.CommandText = "select T1.PO_NO,t1.ORD_AMOUNT,t2.prov_value,t3.SUPL_NAME from RATE_CONTRACT T1 INNER JOIN (select SUPL_ID,PO_NO,sum(prov_amt) as prov_value from mb_book where PO_NO='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "' group by SUPL_ID,PO_NO) as T2
					ON T1.PO_NO = T2.PO_NO join SUPL T3 on t2.SUPL_ID=T3.SUPL_ID"
                End If
            ...
            ElseIf (TextBox3.Text.Substring(0, 1) = "S") Then
                ...
                da = New SqlDataAdapter("DECLARE @TT TABLE(PO_NO VARCHAR(60),W_SLNO VARCHAR(30),vocabNo VARCHAR(30),W_NAME VARCHAR(150),W_AU VARCHAR(30),UNIT_PRICE DECIMAL(16,2),W_START_DATE VARCHAR(30),W_END_DATE VARCHAR(30),W_QTY DECIMAL(16,3))
			INSERT INTO @TT
			select SO_NO as PO_NO,ITEM_SLNO as W_SLNO,ITEM_VOCAB,F_ITEM.ITEM_NAME as W_NAME,MAX(ORD_AU) AS W_AU,SUM(ITEM_UNIT_RATE) AS UNIT_PRICE,MIN(AMD_DATE) AS W_START_DATE,MAX(ITEM_DELIVERY) AS W_END_DATE,SUM(ITEM_QTY) AS W_QTY from SO_MAT_ORDER join F_ITEM on SO_MAT_ORDER.ITEM_CODE=F_ITEM.ITEM_CODE where SO_NO ='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "' GROUP BY SO_NO,ITEM_SLNO,ITEM_VOCAB,ITEM_NAME ORDER BY ITEM_SLNO

			DECLARE @TT1 TABLE(PO_NO VARCHAR(30),W_SLNO VARCHAR(30),W_COMPLETED VARCHAR(30))
			INSERT INTO @TT1
			select SO_NO as PO_NO,MAT_SLNO as WO_SLNO,SUM(TOTAL_QTY) AS W_COMPLETED from DESPATCH where SO_NO='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "' and INV_STATUS<>'CANCELLED' GROUP BY SO_NO,MAT_SLNO
			...
			SELECT T1.PO_NO,T1.W_SLNO,t1.vocabNo,T1.W_NAME,T1.W_AU,t1.UNIT_PRICE,T1.W_START_DATE,T1.W_END_DATE,T1.W_QTY,ISNULL(T2.W_COMPLETED, 0.000) As W_COMPLETED FROM @TT T1 LEFT JOIN @TT1 T2 ON T1.PO_NO=T2.PO_NO AND T1.W_SLNO=T2.W_SLNO order by t1.PO_NO,convert(int,T1.W_SLNO)", conn)
                ...
                Dim mc1 As New SqlCommand
                conn.Open()
                mc1.CommandText = "select dater.d_name as supl_name from dater join ORDER_DETAILS on ORDER_DETAILS.PARTY_CODE=d_code where ORDER_DETAILS.SO_NO  ='" & TextBox3.Text.Substring(0, TextBox3.Text.IndexOf(",") - 1) & "'"
                mc1.Connection = conn
                dr = mc1.ExecuteReader
                If dr.HasRows Then
                    dr.Read()

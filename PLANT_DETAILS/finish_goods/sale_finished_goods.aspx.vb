...
Public Class sale_finished_goods
    ...
    Protected Sub DropDownList5_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DropDownList5.SelectedIndexChanged
        ...
        If MAT_TYPE = "FINISH GOODS" Then
            ...
            da = New SqlDataAdapter("select MAX(SO_MAT_ORDER.ITEM_CODE) AS ITEM_CODE ,MAX(F_ITEM .ITEM_NAME) AS ITEM_NAME,MAX(F_ITEM .ITEM_AU) AS ITEM_AU,MAX(SO_MAT_ORDER.ITEM_WEIGHT) AS ITEM_WEIGHT,SUM(SO_MAT_ORDER .ITEM_QTY) AS ITEM_QTY,(SUM(SO_MAT_ORDER.ITEM_QTY)-SUM(SO_MAT_ORDER.ITEM_QTY_SEND)) AS ITEM_BAL_QTY,MAX(F_ITEM .ITEM_F_STOCK) AS ITEM_F_STOCK,convert(decimal(10,3) ,((MAX(F_ITEM .ITEM_F_STOCK) * MAX(SO_MAT_ORDER.ITEM_WEIGHT)) / 1000)) AS ITEM_F_STOCK_MT FROM SO_MAT_ORDER JOIN F_ITEM ON SO_MAT_ORDER .ITEM_CODE = F_ITEM .ITEM_CODE where SO_MAT_ORDER .ITEM_CODE='" & DropDownList5.Text & "' AND SO_MAT_ORDER.SO_NO='" & TextBox124.Text & "' AND SO_MAT_ORDER.ITEM_SLNO='" & DropDownList4.Text.Substring(0, DropDownList4.Text.IndexOf(",") - 1).Trim & "'", conn)
            da.Fill(dt)
            conn.Close()

...
Public Class DeliveryChallan
    ...
    Protected Sub DropDownList5_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DropDownList5.SelectedIndexChanged
        ...
        If MAT_TYPE = "FINISH GOODS" Then
            ...
            da = New SqlDataAdapter("select MAX(SO_MAT_ORDER.ITEM_CODE) AS ITEM_CODE ,MAX(F_ITEM .ITEM_NAME) AS ITEM_NAME,MAX(F_ITEM .ITEM_AU) AS ITEM_AU,MAX(SO_MAT_ORDER.ITEM_WEIGHT) AS ITEM_WEIGHT,SUM(SO_MAT_ORDER .ITEM_QTY) AS ITEM_QTY,(SUM(SO_MAT_ORDER.ITEM_QTY)-SUM(SO_MAT_ORDER.ITEM_QTY_SEND)) AS ITEM_BAL_QTY,MAX(F_ITEM .ITEM_F_STOCK) AS ITEM_F_STOCK,convert(decimal(10,3) ,((MAX(F_ITEM .ITEM_F_STOCK) * MAX(SO_MAT_ORDER.ITEM_WEIGHT)) / 1000)) AS ITEM_F_STOCK_MT, SUM(SO_MAT_ORDER .ITEM_UNIT_RATE) AS ITEM_UNIT_RATE FROM SO_MAT_ORDER JOIN F_ITEM ON SO_MAT_ORDER .ITEM_CODE = F_ITEM .ITEM_CODE where SO_MAT_ORDER .ITEM_CODE='" & DropDownList5.Text & "' AND SO_MAT_ORDER.SO_NO='" & TextBox124.Text & "' AND SO_MAT_ORDER.ITEM_SLNO='" & DropDownList4.Text.Substring(0, DropDownList4.Text.IndexOf(",") - 1).Trim & "'", conn)
            da.Fill(dt)
            conn.Close()
        ...
        txtUnitPrice.Text = DataBinder.GetPropertyValue(FormView1.DataItem, "ITEM_UNIT_RATE")
    ...
    Protected Sub Button35_Click(sender As Object, e As EventArgs) Handles Button35.Click
        ...
        Dim cas4_price, item_qty, unit_rate, discount, pack_forwd, item_cess, item_tcs, terminal_tax, cgst, sgst, igst, qty_send, freight_rate, UNIT_WEIGHT As Decimal
        ...
        freight_rate = 0
        conn.Open()
        mc1.CommandText = "select ITEM_WEIGHT,ITEM_F_STOCK from F_ITEM where ITEM_CODE='" & DropDownList5.Text & "'"
        mc1.Connection = conn
        dr = mc1.ExecuteReader
        If dr.HasRows Then
            dr.Read()
        ...
        If (DropDownList6.SelectedItem.Text <> "PARTY" And DropDownList6.SelectedItem.Text <> "N/A") Then
            ...
            If (W_AU = "Vehicle") Then
                ...
                End If
            Else
                If WO_Qty < Work_completed + FormatNumber(CDec(TextBox7.Text), 3) Then
                    TextBox54.Focus()
    ...
    Protected Sub Button36_Click(sender As Object, e As EventArgs) Handles Button36.Click
        Using conn_trans
            ...
            Try
                ...
                inv_type = "Delivery Challan"
                inv_rule = "(Under Rule 55 of CGST)"
                ...
                total_rate_unit = txtUnitPrice.Text
                ...
                values(@MATERIAL_TYPE,@TRANSPORTER_WEIGHT,@QUALITY,@INV_STATUS,@SO_NO ,@SO_DATE ,@PO_NO ,@PO_DATE ,@AMD_NO ,@AMD_DATE ,@TRANS_WO ,@TRANS_SLNO ,@TRANS_NAME ,@TRUCK_NO ,@PARTY_CODE ,@CONSIGN_CODE ,@MAT_VOCAB ,@MAT_SLNO ,@INV_NO,@INV_DATE,@D_TYPE,@CHPTR_HEAD ,@FISCAL_YEAR,@INV_ISSUE ,@PLACE_OF_SUPPLY ,@BILL_PARTY_ADD ,@CON_PARTY_ADD ,@B_STATE ,@B_STATE_CODE ,@C_STATE ,@C_STATE_CODE ,@BILL_PARTY_GST_N ,@CON_PARTY_GST_N ,@NEGOTIATING_BRANCH ,@PAYING_AUTH ,@RR_NO ,@RR_DATE ,@TOTAL_WEIGHT ,@ACC_UNIT ,@PURITY ,@TC_NO ,@FINANCE_ARRENGE ,@MILL_CODE ,@DA_NO ,@CONTRACT_NO ,@RCD_VOUCHER_NO ,@RCD_VOUCHER_DATE ,@ROUT_CARD_NO ,@DESPATCH_TYPE ,@TAX_REVERS_CHARGE ,@RLY_INV_NO ,@RLY_INV_DATE ,@FRT_WT_AMT ,@TOTAL_PCS,@P_CODE ,@P_DESC ,@D1 ,@D2 ,@D3 ,@D4 ,@BASE_PRICE ,@PACK_PRICE ,@PACK_TYPE ,@QLTY_PRICE ,@SEC_PRICE ,@TOTAL_TDC ,@UNIT_PRICE ,@SY_MARGIN ,@PPM_FRT ,@FRT_TYPE ,@RLY_ROAD_FRT ,@TOTAL_RATE_UNIT ,@REBATE_UNIT ,@REBATE_TYPE ,@TAXABLE_RATE_UNIT ,@TOTAL_QTY ,@TAXABLE_VALUE ,@CGST_RATE ,@CGST_AMT ,@SGST_RATE ,@SGST_AMT ,@IGST_RATE ,@IGST_AMT ,@CESS_RATE ,@CESS_AMT ,@TERM_RATE ,@TERM_AMT ,@TOTAL_AMT ,@LESS_LOAD_AMT ,@TOTAL_BAG ,@ADVANCE_PAID ,@GST_PAID_ADV ,@NET_PAY ,@NOTIFICATION_TEXT ,@COMM ,@DIV_ADD ,@INV_TYPE ,@INV_RULE ,@FORM_NAME,@EMP_ID, @TCS_TAX, @TCS_AMT)"
                ...
                cmd1.Parameters.AddWithValue("@MATERIAL_TYPE", "FINISHED GOODS")
                cmd1.ExecuteReader()
                cmd1.Dispose()
                'conn.Close()
                ...
                For lp = 0 To GridView2.Rows.Count - 1
                    ...
                    'Update F_item
                    QUARY1 = "update F_ITEM set ITEM_F_STOCK =ITEM_F_STOCK - " & sendqty & " ,ITEM_LAST_DESPATCH = @ITEM_LAST_DESPATCH where ITEM_CODE ='" & GridView2.Rows(lp).Cells(1).Text & "'"
                    ...
                    fr_stock = 0
                    bsr_stock = 0
                    mcc.CommandText = "select (case when (ITEM_F_STOCK) is null then '0' else (ITEM_F_STOCK) end ) as fr_stock ,(case when (ITEM_B_STOCK) is null then '0' else (ITEM_B_STOCK) end ) as bsr_stock from F_ITEM WITH(NOLOCK) where ITEM_CODE='" & GridView2.Rows(lp).Cells(1).Text & "' "
                    mcc.Connection = conn
                    ...
                    conn.Close()
                    'conn.Open()
                    QUARY1 = "Insert Into PROD_CONTROL(TRANSACTION_TYPE,TOTAL_WEIGHT,ITEM_WEIGHT,QUALITY,ENTRY_DATE,FISCAL_YEAR,INV_NO,ITEM_CODE,PROD_DATE,ITEM_F_QTY,ITEM_B_QTY,ITEM_I_QTY,ITEM_I_SO,ITEM_F_STOCK,ITEM_B_STOCK,ITEM_I_TOTAL)values(@TRANSACTION_TYPE,@TOTAL_WEIGHT,@ITEM_WEIGHT,@QUALITY,@ENTRY_DATE,@FISCAL_YEAR,@INV_NO,@ITEM_CODE,@PROD_DATE,@ITEM_F_QTY,@ITEM_B_QTY,@ITEM_I_QTY,@ITEM_I_SO,@ITEM_F_STOCK,@ITEM_B_STOCK,@ITEM_I_TOTAL)"
                    ...
                    cmd1.Parameters.AddWithValue("@TRANSACTION_TYPE", "DESPATCH")
                    cmd1.Parameters.AddWithValue("@TOTAL_WEIGHT", weight)
                    cmd1.ExecuteReader()
                    cmd1.Dispose()
                    'conn.Close()
                ...
                '            Dim EinvErrorData As List(Of EinvoiceErrorDetailsClassEY) = logicClassObj.GenerateEwayBillOnly(AuthErrorData.Item(0).Idtoken, Guid.NewGuid().ToString(), "", TextBox177.Text + TextBox65.Text, TextBox125.Text, TextBox18.Text, "NO", tcsFlag)
                ...
                '            Dim EinvErrorData As List(Of EinvoiceErrorDetailsClassEY) = logicClassObj.GenerateEInvoice(AuthErrorData.Item(0).Idtoken, Guid.NewGuid().ToString(), "", TextBox177.Text + TextBox65.Text, TextBox125.Text, TextBox18.Text, "NO", tcsFlag, "INV")

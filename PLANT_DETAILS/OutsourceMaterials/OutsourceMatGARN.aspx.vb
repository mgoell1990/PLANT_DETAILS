...
Public Class OutsourceMatGARN
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            conn.Open()
            da = New SqlDataAdapter("select distinct GARN_NO from PO_RCD_MAT where GARN_NO LIKE 'OGARN%' ORDER BY GARN_NO", conn)
            ...
            dt5.Columns.AddRange(New DataColumn(9) {New DataColumn("ITEM_NAME"), New DataColumn("SUPL_NAME"), New DataColumn("BE_NO"), New DataColumn("MAT_CODE"), New DataColumn("PO_NO"), New DataColumn("TRANS_WO_NO"), New DataColumn("BE_QTY"), New DataColumn("BE_BAL"), New DataColumn("amd_no"), New DataColumn("CHA_ORDER")})
            ViewState("IMP") = dt5
            BINDGRID3()
            dt5.Rows.Add("", "", "", "", "", "", "", "", "", "")
            ViewState("IMP") = dt5
    ...
    Protected Sub new_button_Click(sender As Object, e As EventArgs) Handles new_button.Click
        If type_DropDown.SelectedValue = "OUTSOURCED ITEMS" Then
            ...
            ViewState("mat2") = dt2
            Me.BINDGRID2()
        ElseIf type_DropDown.SelectedValue = "OUTSOURCED ITEMS(IMP)" Then
            Dim ds5 As New DataSet
            conn.Open()
            da = New SqlDataAdapter("select distinct PO_RCD_MAT.CRR_NO from PO_RCD_MAT JOIN ORDER_DETAILS ON PO_RCD_MAT.PO_NO=ORDER_DETAILS.SO_NO where PO_RCD_MAT.GARN_NO='PENDING' AND PO_RCD_MAT.MAT_RCD_QTY > PO_RCD_MAT.MAT_REJ_QTY AND PO_RCD_MAT.INSP_EMP IS NOT NULL AND ORDER_DETAILS.PO_TYPE='" & type_DropDown.SelectedValue & "' ORDER BY CRR_NO", conn)
            da.Fill(ds5, "PO_RCD_MAT")
            imp_crr_no_DropDownList1.Items.Clear()
    ...
    Protected Sub imp_mat_slno_DropDownList2_SelectedIndexChanged(sender As Object, e As EventArgs) Handles imp_mat_slno_DropDownList2.SelectedIndexChanged
        If imp_mat_slno_DropDownList2.SelectedValue = "Select" Then
            ...
            Dim dt2 As New DataTable()
            dt2.Columns.AddRange(New DataColumn(9) {New DataColumn("ITEM_NAME"), New DataColumn("SUPL_NAME"), New DataColumn("BE_NO"), New DataColumn("ITEM_CODE"), New DataColumn("PO_NO"), New DataColumn("TRANS_WO_NO"), New DataColumn("BE_QTY"), New DataColumn("BE_BAL"), New DataColumn("amd_no"), New DataColumn("CHA_ORDER")})
            ViewState("IMP") = dt2
            BINDGRID3()
        ...
        End If
        conn.Open()
        da = New SqlDataAdapter("select PO_RCD_MAT.amd_no,BE_DETAILS.CHA_ORDER, Outsource_F_ITEM.ITEM_NAME ,SUPL.SUPL_NAME ,BE_DETAILS .BE_NO ,PO_RCD_MAT .PO_NO , PO_RCD_MAT .MAT_CODE ,BE_DETAILS .BE_QTY,PO_RCD_MAT .TRANS_WO_NO ,(BE_DETAILS .BE_QTY -BE_DETAILS .RCVD_QTY ) as BE_BAL from PO_RCD_MAT join Outsource_F_ITEM on PO_RCD_MAT .MAT_CODE =Outsource_F_ITEM.ITEM_CODE join BE_DETAILS on PO_RCD_MAT .PO_NO = BE_DETAILS .PO_NO and PO_RCD_MAT .MAT_SLNO =BE_DETAILS .MAT_SLNO and PO_RCD_MAT .BE_NO =BE_DETAILS .BE_NO   join supl on PO_RCD_MAT .SUPL_ID =SUPL.SUPL_ID  WHERE PO_RCD_MAT.CRR_NO ='" & imp_crr_no_DropDownList1.SelectedValue & "' AND PO_RCD_MAT .MAT_SLNO =" & imp_mat_slno_DropDownList2.SelectedValue, conn)
        da.Fill(dt)
        conn.Close()
        ViewState("IMP") = dt
        BINDGRID3()
    ...
    Protected Sub imp_crr_no_DropDownList1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles imp_crr_no_DropDownList1.SelectedIndexChanged
        ...
        Else
            ...
            conn.Open()
            da = New SqlDataAdapter("select PO_RCD_MAT.MAT_SLNO from po_rcd_mat join po_ord_mat on PO_RCD_MAT .po_no=PO_ORD_MAT .po_no and PO_RCD_MAT .mat_slno=PO_ORD_MAT .mat_slno and PO_ORD_MAT.amd_no=PO_RCD_MAT.amd_no JOIN Outsource_F_ITEM ON Outsource_F_ITEM.ITEM_CODE =PO_RCD_MAT .MAT_CODE where PO_RCD_MAT .crr_no='" & imp_crr_no_DropDownList1.Text & "' and PO_RCD_MAT.GARN_NO = 'Pending'", conn)
            da.Fill(dt)
            conn.Close()
            ...
            conn.Open()
            dt.Clear()
            da = New SqlDataAdapter("select PO_RCD_MAT .BE_NO, PO_RCD_MAT.PO_NO,PO_RCD_MAT.CRR_NO,PO_RCD_MAT.CHLN_NO,PO_RCD_MAT.MAT_SLNO,PO_RCD_MAT.MAT_CODE,Outsource_F_ITEM.ITEM_NAME,Outsource_F_ITEM.ITEM_AU,PO_ORD_MAT.MAT_QTY,PO_RCD_MAT.MAT_CHALAN_QTY,PO_RCD_MAT.MAT_RCD_QTY,PO_RCD_MAT.MAT_REJ_QTY,PO_RCD_MAT.MAT_EXCE,PO_RCD_MAT.MAT_BAL_QTY,PO_RCD_MAT.INSP_NOTE,PO_RCD_MAT.CRR_DATE,PO_RCD_MAT.AMD_NO from po_rcd_mat join po_ord_mat on PO_RCD_MAT .po_no=PO_ORD_MAT .po_no and PO_RCD_MAT .mat_slno=PO_ORD_MAT .mat_slno and PO_ORD_MAT.amd_no=PO_RCD_MAT.amd_no JOIN Outsource_F_ITEM ON Outsource_F_ITEM.ITEM_CODE =PO_RCD_MAT .MAT_CODE where PO_RCD_MAT .crr_no='" & imp_crr_no_DropDownList1.Text & "' and PO_RCD_MAT.GARN_NO = 'Pending' AND MAT_RCD_QTY <> MAT_REJ_QTY ORDER BY PO_RCD_MAT.MAT_SLNO", conn)
            da.Fill(dt)
            conn.Close()
    ...
    Protected Sub Button50_Click(sender As Object, e As EventArgs) Handles Button50.Click
        ...
        If (CDate(working_date) <= CDate(Block_DATE)) Then
            Label534.Visible = True
            Label534.Text = "Garn before " & Block_DATE & " has been freezed."
        ...
        Else
            ...
            If (CRR_DATE > CDate(working_date)) Then
                Label534.Visible = True
            ...
            Else
                ...
                For I = 0 To imp_GridView3.Rows.Count - 1
                    If imp_GridView3.Rows(I).Cells(3).Text = imp_mat_slno_DropDownList2.SelectedValue Then
                        Dim mc1 As New SqlCommand
                        Dim TRANS_WO, TRANS_SLNO, cha_wo, cha_wo_sl As String
                        Dim MAT_RCVD_QTY, MAT_CHLN_QTY, QTY_TO_BE_CONSIDERED As New Decimal(0)
                        TRANS_WO = ""
                        TRANS_SLNO = ""
                        cha_wo = ""
                        cha_wo_sl = ""
                        ...
                        If TRANS_WO = "N/A" Then
                            ...
                            QTY_TO_BE_CONSIDERED = 0.00
                            If (BE_QTY >= BE_RCVD_QTY + MAT_RCVD_QTY) Then
                                MAT_RCVD_QTY = CDec(imp_GridView3.Rows(I).Cells(12).Text)
                                QTY_TO_BE_CONSIDERED = CDec(imp_GridView3.Rows(I).Cells(12).Text)
                            Else
                                ...
                                Else
                                    ...
                                    QTY_TO_BE_CONSIDERED = MAT_RCVD_QTY - (BE_RCVD_QTY + MAT_RCVD_QTY - BE_QTY)
                                End If
                            ...
                            'CHA_FINAL_PRICE = (CHA_PRICE - CHA_DISCOUNT) * CDec(imp_GridView3.Rows(I).Cells(12).Text)
                        ...
                        Else
                            ...
                            MAT_RCVD_QTY = CDec(imp_GridView3.Rows(I).Cells(12).Text)
                            MAT_CHLN_QTY = CDec(imp_GridView3.Rows(I).Cells(9).Text)
                            QTY_TO_BE_CONSIDERED = 0.00
                            ''Checking if quantity is exceeding the BE quantity

                            If (BE_QTY >= BE_RCVD_QTY + MAT_CHLN_QTY) Then
                                MAT_RCVD_QTY = CDec(imp_GridView3.Rows(I).Cells(12).Text)
                                QTY_TO_BE_CONSIDERED = CDec(imp_GridView3.Rows(I).Cells(9).Text)
                            Else
                                If ((BE_RCVD_QTY + MAT_CHLN_QTY - BE_QTY) > MAT_CHLN_QTY) Then
                                    MAT_RCVD_QTY = 0.00
                                    QTY_TO_BE_CONSIDERED = 0.00
                                Else
                                    MAT_RCVD_QTY = MAT_CHLN_QTY - (BE_RCVD_QTY + MAT_CHLN_QTY - BE_QTY)
                                    QTY_TO_BE_CONSIDERED = MAT_CHLN_QTY - (BE_RCVD_QTY + MAT_CHLN_QTY - BE_QTY)
                                End If

                            End If

                            unit_value = FormatNumber(UNIT_PRICE, 2)
                            mat_value = FormatNumber(((unit_value) * QTY_TO_BE_CONSIDERED), 2)
                            cenvat_value = (UNIT_CENVAT * QTY_TO_BE_CONSIDERED)
                            Dim MC As New SqlCommand
                            Dim CHA_PRICE, CHA_DISCOUNT, CHA_FINAL_PRICE As Decimal
                            CHA_PRICE = 0
                            CHA_DISCOUNT = 0
                            ...
                            conn.Close()
                            CHA_DISCOUNT = (CHA_PRICE * CHA_DISCOUNT) / 100
                            'CHA_FINAL_PRICE = (CHA_PRICE - CHA_DISCOUNT) * CDec(imp_GridView3.Rows(I).Cells(9).Text)
                            CHA_FINAL_PRICE = (CHA_PRICE - CHA_DISCOUNT) * QTY_TO_BE_CONSIDERED
                            CHA_VALUE = FormatNumber(CHA_FINAL_PRICE, 2)
                            'CHA_RCD = FormatNumber((CHA_PRICE - CHA_DISCOUNT) * CDec(imp_GridView3.Rows(I).Cells(12).Text), 2)
                            conn.Open()
                            ...
                            If tole_weight <= TRANS_DIFF Then
                                ...
                                trans_short_value = FormatNumber((TRANS_DIFF) * ((mat_value + CHA_VALUE) / CHLN_QTY), 2)
                                TRANS_QTY = CDec(imp_GridView3.Rows(I).Cells(10).Text)
                            Else
                        ...
                        imp_GridView3.Rows(I).Cells(26).Text = FormatNumber((CUSTOM_DUTY / BE_QTY) * QTY_TO_BE_CONSIDERED, 2)
                        imp_GridView3.Rows(I).Cells(27).Text = FormatNumber((IGST / BE_QTY) * QTY_TO_BE_CONSIDERED, 2)
                        imp_GridView3.Rows(I).Cells(28).Text = FormatNumber((STATUTORY_CHARGE / BE_QTY) * QTY_TO_BE_CONSIDERED, 2)
                        imp_GridView3.Rows(I).Cells(29).Text = FormatNumber((INSURANCE / BE_QTY) * QTY_TO_BE_CONSIDERED, 2)
                        If (BE_QTY >= BE_RCVD_QTY + MAT_CHLN_QTY) Then
                            imp_GridView3.Rows(I).Cells(30).Text = "0.00"
                        Else
                            imp_GridView3.Rows(I).Cells(30).Text = FormatNumber(BE_RCVD_QTY + MAT_CHLN_QTY - BE_QTY, 2)
                        End If
                    End If
                Next
            ...
            Try
                ...
                For I = 0 To imp_GridView3.Rows.Count - 1
                    If IsNumeric(imp_GridView3.Rows(I).Cells(17).Text) = False Then
                        Label534.Visible = True
                        Label534.Text = "Please Add Item Sl No Wise Details"
                        Return
                    Else
                ...
                For I = 0 To imp_GridView3.Rows.Count - 1
                    ...
                    'PURCHASE_VALUE = CDec(imp_GridView3.Rows(I).Cells(17).Text) + CDec(imp_GridView3.Rows(I).Cells(26).Text) + CDec(imp_GridView3.Rows(I).Cells(28).Text) + CDec(imp_GridView3.Rows(I).Cells(18).Text) + CDec(imp_GridView3.Rows(I).Cells(29).Text)
                    ...
                    INV_NO = ""
                    conn.Open()
                    mycommand.CommandText = "SELECT BE_DETAILS .BE_NO ,BE_DETAILS .INV_NO ,BE_DETAILS .INV_DATE  FROM BE_DETAILS JOIN PO_RCD_MAT ON BE_DETAILS .BE_NO =PO_RCD_MAT .BE_NO AND BE_DETAILS .MAT_SLNO =PO_RCD_MAT .MAT_SLNO WHERE PO_RCD_MAT .CRR_NO ='" & imp_GridView3.Rows(I).Cells(0).Text & "'AND PO_RCD_MAT .MAT_SLNO =" & imp_GridView3.Rows(I).Cells(3).Text
                    mycommand.Connection = conn
                    dr = mycommand.ExecuteReader
                    If dr.HasRows Then
                        dr.Read()
                    ...
                    NEW_MAT_VALUE = PROV_MAT_VALUE
                    NEW_UNIT_RATE = CDec(FormatNumber(NEW_MAT_VALUE / CDec(imp_GridView3.Rows(I).Cells(12).Text) + CDec(imp_GridView3.Rows(I).Cells(13).Text), 2))
                    NEW_AVG_PRICE = FormatNumber(((STOCK_QTY * AVG_PRICE) + NEW_MAT_VALUE) / (CDec(imp_GridView3.Rows(I).Cells(12).Text) + CDec(imp_GridView3.Rows(I).Cells(13).Text) + STOCK_QTY), 2)

                    'conn.Open()
                    Dim Query As String
                    Dim CMD As New SqlCommand
                    Query = "update Outsource_F_ITEM set MAT_AVG=@MAT_AVG,LAST_TRANS_DATE=@LAST_TRANS_DATE,ITEM_F_STOCK=@ITEM_F_STOCK,MAT_LAST_RATE=@MAT_LAST_RATE,MAT_LASTPUR_DATE=@MAT_LASTPUR_DATE where ITEM_CODE='" & imp_GridView3.Rows(I).Cells(4).Text & "'"
                    CMD = New SqlCommand(Query, conn_trans, myTrans)
                    CMD.Parameters.AddWithValue("@MAT_AVG", NEW_AVG_PRICE)
                    CMD.Parameters.AddWithValue("@ITEM_F_STOCK", CDec(imp_GridView3.Rows(I).Cells(12).Text) + CDec(imp_GridView3.Rows(I).Cells(13).Text) + STOCK_QTY)
                    ...
                    conn.Open()
                    ds.Clear()
                    da = New SqlDataAdapter("select LINE_NO from MAT_DETAILS WHERE FISCAL_YEAR =" & CInt(STR1) & " AND MAT_CODE ='" & imp_GridView3.Rows(I).Cells(4).Text & "' AND LINE_NO <> 0", conn)
                    ...
                    CMD.Parameters.AddWithValue("@MAT_QTY", CDec(imp_GridView3.Rows(I).Cells(12).Text) + CDec(imp_GridView3.Rows(I).Cells(13).Text))
                    CMD.Parameters.AddWithValue("@MAT_BALANCE", CDec(imp_GridView3.Rows(I).Cells(12).Text) + CDec(imp_GridView3.Rows(I).Cells(13).Text) + STOCK_QTY)
                    ...
                    'CUSTOM DUTY credit
                    LEDGER_SAVE_PUR(po_no, imp_GridView3.Rows(I).Cells(3).Text, "SC069", imp_garn_no_TextBox1.Text, "86201", "Cr", FormatNumber(CUSTOM_DUTY + IGST, 2), "CUSTOM DUTY", 4, "", imp_GridView3.Rows(I).Cells(24).Text)
                    ...
                    If dr.HasRows Then
                        ...
                        taxable_value = Math.Round(IGST * 100 / IGST_PERCENTAGE, 2)
                    ...
                    CMD.Parameters.AddWithValue("@INVOICE_NO", imp_garn_no_TextBox1.Text)
                ...
                For C = 0 To imp_GridView3.Rows.Count - 1
                    ...
                    conn.Open()
                    Dim MC55 As New SqlCommand
                    MC55.CommandText = "SELECT NO_OF_BAG,BAG_WEIGHT FROM PO_RCD_MAT WITH(NOLOCK) WHERE CRR_NO = '" & imp_GridView3.Rows(C).Cells(0).Text & "' AND MAT_SLNO=" & imp_GridView3.Rows(C).Cells(3).Text
                    MC55.Connection = conn
                    dr = MC55.ExecuteReader
                    If dr.HasRows Then
                        dr.Read()
                ...
                If PROV_PRICE_FOR_TRANSPORTER > 0 Then
                    ...
                    MC9.CommandText = "select * FROM mb_book WHERE mb_no='" & imp_crr_no_DropDownList1.SelectedValue & "'"
                    MC9.Connection = conn
                ...
                'MC10.CommandText = "select * from work_group where work_type = (SELECT PO_TYPE FROM ORDER_DETAILS WHERE SO_NO='" & po_no & "')"
                'MC10.Connection = conn
                'dr = MC10.ExecuteReader
                'If dr.HasRows Then
                '    dr.Read()
                ...
                'End If
                'conn.Close()
                LEDGER_SAVE_PUR(po_no, 0, "", imp_garn_no_TextBox1.Text, cenvat_head, "Dr", IGST, "IGST", 2, "", imp_GridView3.Rows(0).Cells(24).Text)
                '' LEDGER_SAVE_PUR(po_no, 0, "", imp_garn_no_TextBox1.Text, entry_tax_head, "Cr", ENTRY_TAX, "ENTRY TAX", 6, "")

                Label534.Visible = True
                Label534.Text = "GARN Data Added"
                ''GRID VIEW CLEAR
                Dim dt4 As New DataTable()
                dt4.Columns.AddRange(New DataColumn(13) {New DataColumn("PO No"), New DataColumn("Mat Sl No"), New DataColumn("Mat Code"), New DataColumn("Mat Name"), New DataColumn("A/U"), New DataColumn("Ord Qty"), New DataColumn("Chln Qty"), New DataColumn("Rcvd Qty"), New DataColumn("Rej Qty"), New DataColumn("Acept Qty"), New DataColumn("Exces Qty"), New DataColumn("Bal Qty"), New DataColumn("Note"), New DataColumn("BE_NO")})
                ViewState("imp_mat") = dt4
    ...
    Protected Sub garn_crrnoDropDownList_SelectedIndexChanged(sender As Object, e As EventArgs) Handles garn_crrnoDropDownList.SelectedIndexChanged
        If garn_crrnoDropDownList.SelectedValue <> "Select" Then
            conn.Open()
            da = New SqlDataAdapter("select PO_RCD_MAT.MAT_SLNO from po_rcd_mat join po_ord_mat on PO_RCD_MAT .po_no=PO_ORD_MAT .po_no and PO_RCD_MAT .mat_slno=PO_ORD_MAT .mat_slno and PO_ORD_MAT.amd_no=PO_RCD_MAT.amd_no JOIN Outsource_F_ITEM ON Outsource_F_ITEM.ITEM_CODE =PO_RCD_MAT .MAT_CODE where PO_RCD_MAT .crr_no='" & garn_crrnoDropDownList.Text & "' and PO_RCD_MAT.GARN_NO = 'Pending' AND MAT_RCD_QTY <> MAT_REJ_QTY ORDER BY PO_RCD_MAT.MAT_SLNO", conn)
            da.Fill(dt)
            conn.Close()
        ...
        conn.Open()
        dt.Clear()
        da = New SqlDataAdapter("select PO_RCD_MAT.PO_NO,PO_RCD_MAT.TOTAL_MT,PO_RCD_MAT.CRR_NO,PO_RCD_MAT.CHLN_NO,PO_RCD_MAT.MAT_SLNO,PO_RCD_MAT.MAT_CODE,Outsource_F_ITEM.ITEM_NAME,Outsource_F_ITEM.ITEM_AU,PO_ORD_MAT.MAT_QTY,PO_RCD_MAT.MAT_CHALAN_QTY,PO_RCD_MAT.MAT_RCD_QTY,PO_RCD_MAT.MAT_REJ_QTY,PO_RCD_MAT.MAT_EXCE,PO_RCD_MAT.MAT_BAL_QTY,PO_RCD_MAT.INSP_NOTE,PO_RCD_MAT.CRR_DATE,PO_RCD_MAT.AMD_NO from po_rcd_mat join po_ord_mat on PO_RCD_MAT .po_no=PO_ORD_MAT .po_no and PO_RCD_MAT .mat_slno=PO_ORD_MAT .mat_slno and PO_ORD_MAT.amd_no=PO_RCD_MAT.amd_no JOIN Outsource_F_ITEM ON Outsource_F_ITEM.ITEM_CODE =PO_RCD_MAT .MAT_CODE where PO_RCD_MAT .crr_no='" & garn_crrnoDropDownList.SelectedValue & "' and PO_RCD_MAT.GARN_NO = 'Pending' AND MAT_RCD_QTY <> MAT_REJ_QTY ORDER BY PO_RCD_MAT.MAT_SLNO", conn)
        da.Fill(dt)
        conn.Close()
        ...
        Dim mc2 As New SqlCommand
        mc2.CommandText = "SELECT SUPL .SUPL_NAME,SUPL.SUPL_ID ,PO_RCD_MAT .TRANS_SLNO, PO_RCD_MAT .PO_NO,PO_RCD_MAT.TRANS_WO_NO  FROM PO_RCD_MAT JOIN SUPL ON PO_RCD_MAT .SUPL_ID =SUPL.SUPL_ID WHERE PO_RCD_MAT.CRR_NO ='" & garn_crrnoDropDownList.SelectedValue & "'"
        mc2.Connection = conn
        dr = mc2.ExecuteReader
        If dr.HasRows Then
            dr.Read()
    ...
    Protected Sub Button44_Click(sender As Object, e As EventArgs) Handles Button44.Click
        ...
        If TextBox2.Visible Then
            ...
            ElseIf IsDate(TextBox2.Text) = False Then
                TextBox2.Focus()
                GARN_ERR_LABLE.Visible = True
                GARN_ERR_LABLE.Text = "Please Enter valid GARN date."
                Return
            End If

        End If

        If TextBox143.Text = "" Or IsNumeric(TextBox143.Text) = False Then
            TextBox143.Focus()
            GARN_ERR_LABLE.Visible = True
            GARN_ERR_LABLE.Text = "Please Enter L.D. Value"
            Return
        ...
        Using conn_trans
            ...
            Try
                ...
                Else
                    ...
                    conn.Open()
                    Dim MC1 As New SqlCommand
                    MC1.CommandText = "SELECT CRR_DATE FROM PO_RCD_MAT WHERE CRR_NO='" & garn_crrnoDropDownList.SelectedValue & "'"
                    MC1.Connection = conn
                    dr = MC1.ExecuteReader
                    If dr.HasRows Then
                        dr.Read()
                    ...
                    Else
                        ...
                        For I = 0 To GridView2.Rows.Count - 1
                            If GridView2.Rows(I).Cells(3).Text = DropDownList6.SelectedValue Then
                                'CALCULATE Outsource_F_ITEM VALUE
                                Dim ACCEPT_QTY, CHLN_QTY, RCVD_QTY, REJ_QTY, PARTY_QTY, excess_qty, TRANS_QTY, TOTAL_MT, PARTY_AMOUNT, TRANSPORT_ADJUST, tolerance As New Decimal(0.0)
                                ...
                                Dim MC As New SqlCommand
                                conn.Open()
                                MC.CommandText = "select TOLERANCE from ORDER_DETAILS where SO_NO = '" & Label398.Text & "'"
                                MC.Connection = conn
                                dr = MC.ExecuteReader
                                If dr.HasRows Then
                                    dr.Read()
                                ...
                                If (GridView2.Rows(I).Cells(6).Text = "PCS") Then
                                    TOLE_VALUE = 0
                                ...
                                cgst_price = FormatNumber((((net_price + freight_price)) * cgst) / 100, 0)
                                sgst_price = FormatNumber((((net_price + freight_price)) * sgst) / 100, 0)
                                igst_price = FormatNumber((((net_price + freight_price)) * igst) / 100, 0)
                                cess_price = FormatNumber((((net_price + freight_price)) * cess) / 100, 0)
                                ''LOCAL FREIGHT
                                ...
                                If Label424.Text <> "N/A" Then
                                    Dim qty, rqty, sqty, eqty, final_price, exqty As New Decimal(0)
                                    Dim w_qty, w_completed, PRICE, w_unit_price, W_discount, DISCOUNT_VALUE As New Decimal(0)
                                    ...
                                    If TOLE_VALUE >= TRANS_DIFF Then
                                        ...
                                        If (GridView2.Rows(I).Cells(6).Text = "PCS") Then
                                            ...
                                            ''SHORTAGE VALUE
                                            TRANS_SHORT_VALUE = 0
                                            TRANS_QTY = GridView2.Rows(I).Cells(33).Text
                                        Else
                                            ''LOSS ON TRANSPORT
                                            WT_VAR = FormatNumber(((net_price + freight_price + LOCAL_FREIGHT_PRICE + analitical_price) / PARTY_QTY) * TRANS_DIFF, 2)
                                            ''LOSS ON GST NOT APPLICABLE
                                    ...
                                    Else
                                        If CDec(GridView2.Rows(I).Cells(9).Text) <> CDec(GridView2.Rows(I).Cells(10).Text) Then
                                            If (GridView2.Rows(I).Cells(6).Text = "PCS") Then
                                                ...
                                                TRANS_SHORT_VALUE = FormatNumber(((net_price + freight_price + LOCAL_FREIGHT_PRICE + analitical_price) / PARTY_QTY) * TRANS_DIFF, 2)
                                                TRANS_QTY = GridView2.Rows(I).Cells(33).Text
                                            Else
                                                ''LOSS ON TRANSPORT
                                                WT_VAR = FormatNumber(((net_price + freight_price + LOCAL_FREIGHT_PRICE + analitical_price) / PARTY_QTY) * TOLE_VALUE, 2)
                                ...
                                ''Outsource_F_ITEM VALUE
                                GridView2.Rows(I).Cells(27).Text = FormatNumber((net_price + LOCAL_FREIGHT_PRICE + analitical_price + freight_price - (penality_price)), 2)
                            End If
                        Next
    ...
    Protected Sub Button5_Click(sender As Object, e As EventArgs) Handles Button5.Click
        ...
        ElseIf TextBox175.Text = "" Or IsNumeric(TextBox175.Text) = False Then
            TextBox175.Focus()
            GARN_ERR_LABLE.Text = "Penality For Transporter can't be empty"
        End If
            ...
            Try
                ...
                For I = 0 To GridView2.Rows.Count - 1
                    If IsNumeric(GridView2.Rows(I).Cells(20).Text) = False Then
                        GARN_ERR_LABLE.Visible = True
                        GARN_ERR_LABLE.Text = "Please Add Outsource_F_ITEM Sl No Wise Details"
                        Return
                    Else
                        GARN_ERR_LABLE.Visible = False
                    End If
                ...
                For I = 0 To GridView2.Rows.Count - 1
                    ...
                    Dim tolerance, TOLE_VALUE, TRANS_DIFF, WT_VAR, QTY_TO_BE_CONSIDERED_FOR_PROV As New Decimal(0.00)
                    If Label424.Text <> "N/A" Then

                        QTY_TO_BE_CONSIDERED_FOR_PROV = CDec(GridView2.Rows(I).Cells(9).Text)
                    ...
                    Else
                        ...
                        If (tolerance <> 0) Then
                            ...
                            Else
                                ''TOLERANCE
                                TOLE_VALUE = (CDec(GridView2.Rows(I).Cells(9).Text) * tolerance) / 100
                                ''TRANSPORTATION DIFFRENCE
                                TRANS_DIFF = CDec(GridView2.Rows(I).Cells(9).Text) - CDec(GridView2.Rows(I).Cells(10).Text)
                            End If
                            ...
                            Else
                                ...
                                QTY_TO_BE_CONSIDERED_FOR_PROV = CDec(GridView2.Rows(I).Cells(10).Text)
                    ...
                    cmd.Parameters.AddWithValue("@PROV_VALUE", Math.Round(CDec(GridView2.Rows(I).Cells(27).Text), 2))
                    ...
                    cmd.Parameters.AddWithValue("@LOSS_TRANSPORT", CDec(GridView2.Rows(I).Cells(29).Text))
                    ...
                    'Dim MC As New SqlCommand
                    conn.Open()
                    MC.CommandText = "select * from Outsource_F_ITEM where ITEM_CODE = '" & GridView2.Rows(I).Cells(4).Text & "'"
                    MC.Connection = conn
                    dr = MC.ExecuteReader
                    If dr.HasRows Then
                        dr.Read()
                    ...
                    Dim NEW_AVG_PRICE, NEW_UNIT_RATE, NEW_MAT_VALUE As Decimal
                    'NEW_MAT_VALUE = (CDec(GridView2.Rows(I).Cells(27).Text) + CDec(GridView2.Rows(I).Cells(30).Text))
                    NEW_MAT_VALUE = (CDec(GridView2.Rows(I).Cells(27).Text))
                    NEW_UNIT_RATE = CDec(FormatNumber(NEW_MAT_VALUE / CDec(GridView2.Rows(I).Cells(12).Text), 2))
                    NEW_AVG_PRICE = FormatNumber(((STOCK_QTY * AVG_PRICE) + NEW_MAT_VALUE) / (CDec(GridView2.Rows(I).Cells(12).Text) + STOCK_QTY), 2)
                    'conn.Open()
                    query = "update Outsource_F_ITEM set MAT_AVG=@MAT_AVG,LAST_TRANS_DATE=@LAST_TRANS_DATE,ITEM_F_STOCK=@ITEM_F_STOCK,MAT_LAST_RATE=@MAT_LAST_RATE,MAT_LASTPUR_DATE=@MAT_LASTPUR_DATE where ITEM_CODE='" & GridView2.Rows(I).Cells(4).Text & "'"
                    cmd = New SqlCommand(query, conn_trans, myTrans)
                    cmd.Parameters.AddWithValue("@MAT_AVG", NEW_AVG_PRICE)
                    cmd.Parameters.AddWithValue("@ITEM_F_STOCK", CDec(GridView2.Rows(I).Cells(12).Text) + STOCK_QTY)
                    cmd.Parameters.AddWithValue("@MAT_LAST_RATE", NEW_UNIT_RATE)
                    cmd.Parameters.AddWithValue("@MAT_LASTPUR_DATE", G_DATE)
                    cmd.Parameters.AddWithValue("@LAST_TRANS_DATE", G_DATE)
                    cmd.ExecuteReader()
                    ...
                    conn.Open()
                    ds.Clear()
                    da = New SqlDataAdapter("select LINE_NO from MAT_DETAILS WHERE FISCAL_YEAR =" & CInt(STR1) & " AND MAT_CODE ='" & GridView2.Rows(I).Cells(4).Text & "' AND LINE_NO <> 0", conn)
                    count = da.Fill(ds, "MAT_DETAILS")
                    conn.Close()
                    ...
                    cmd.Parameters.AddWithValue("@GARN_NO_MB_NO", GARN_NO_TextBox.Text)
                    ...
                    cmd.Parameters.AddWithValue("@ENTRY_DATE", Now)
                    cmd.Parameters.AddWithValue("@AC_NO", PURCHASE_HEAD)
                    cmd.Parameters.AddWithValue("@AMOUNT_DR", NEW_MAT_VALUE)
                ...
                For C = 0 To GridView2.Rows.Count - 1
                    ...
                    End If
                    conn.Close()
                    chln_qty_trans = chln_qty_trans + CDec(GridView2.Rows(C).Cells(9).Text) + ((NO_OF_BAG * BAG_WEIGHT) / 1000)
                ...
                If PROV_PRICE_FOR_TRANSPORTER > 0 Then
                    ...
                    '    w_unit_price = dr.Item("W_UNIT_PRICE")
                    '    W_discount = dr.Item("W_DISCOUNT")
                    '    mat_price = dr.Item("W_Outsource_F_ITEM_COST")
                    '    WO_NAME = dr.Item("W_NAME")
                    '    WO_AU = dr.Item("W_AU")
                    '    WO_SUPL_ID = dr.Item("SUPL_ID")
                    '    WO_AMD = dr.Item("WO_AMD")
                    ...
                    conn.Open()
                    Dim v_ind As String = ""
                    MC.CommandText = "select * FROM mb_book WHERE mb_no='" & garn_crrnoDropDownList.SelectedValue & "'"
                    MC.Connection = conn
                    dr = MC.ExecuteReader
                    If dr.HasRows Then
                        dr.Read()
                    ...
                    If (v_ind = "N") Then
                        ...
                        'conn.Open()
                        Dim Query As String = "update mb_book set pen_amt=@pen_amt where mb_no='" & garn_crrnoDropDownList.SelectedValue & "'"
                        Dim cmd As New SqlCommand(Query, conn_trans, myTrans)
                        cmd.Parameters.AddWithValue("@pen_amt", CDec(TextBox175.Text) + SHORT_AMT)
                        cmd.ExecuteReader()
                        cmd.Dispose()
                        'conn.Close()

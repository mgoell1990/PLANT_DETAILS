...
Public Class OutsourceMatCRR
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            type_DropDown.Items.Add("OUTSOURCED ITEMS")
            type_DropDown.Items.Add("OUTSOURCED Items(IMP)")
    ...
    Protected Sub pono_DropDownList_SelectedIndexChanged(sender As Object, e As EventArgs) Handles pono_DropDownList.SelectedIndexChanged
        ...
        conn.Open()
        Dim ds5 As New DataSet
        da = New SqlDataAdapter("select DISTINCT (CONVERT(varchar(10), MAT_SLNO) + ' , ' + MAT_NAME) AS MAT_SLNO from  PO_ORD_MAT where PO_NO='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "' and MAT_STATUS='PENDING'", conn)
        da.Fill(ds5, "PO_ORD_MAT")
        ...
        mc1.CommandText = "SELECT ORDER_DETAILS.DELIVERY_TERM,ORDER_DETAILS.MODE_OF_DESPATCH,SUPL .SUPL_NAME FROM ORDER_DETAILS JOIN SUPL ON ORDER_DETAILS .PARTY_CODE =SUPL.SUPL_ID WHERE ORDER_DETAILS.SO_NO='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "'"
        mc1.Connection = conn
        ...
        If str = "By Customer" Or str = "By Supplier(Door Delivery)" Or str = "N/A" Then
            ...
            TransChalanQty.Enabled = False
            TransRcdQty.Enabled = False
            TransChalanQty.Text = "0.00"
            TransRcdQty.Text = "0.00"
        ...
        Else
            ...
            conn.Open()
            dt.Clear()
            da = New SqlDataAdapter("SELECT distinct WO_ORDER .PO_NO + ' , ' + SUPL .SUPL_NAME AS PO_NO  FROM ORDER_DETAILS JOIN WO_ORDER ON ORDER_DETAILS .SO_NO =WO_ORDER .PO_NO join SUPL on ORDER_DETAILS.PARTY_CODE =SUPL.SUPL_ID WHERE ORDER_DETAILS .PO_TYPE ='FREIGHT INWARD' AND WO_ORDER.W_STATUS='PENDING' and WO_ORDER.W_END_DATE >'" & working_date.Date.Year & "-" & working_date.Date.Month & "-" & working_date.Date.Day & "' AND WO_ORDER.WO_TYPE='FREIGHT INWARD' ORDER BY PO_NO", conn)
            ...
            conn.Close()
            trans_DropDownList.Items.Insert(0, "Select")
    ...
    Protected Sub mat_sl_noDropDownList_SelectedIndexChanged(sender As Object, e As EventArgs) Handles mat_sl_noDropDownList.SelectedIndexChanged
        ...
        Dim mc1 As New SqlCommand
        conn.Open()
        mc1.CommandText = "SELECT PO_ORD_MAT .MAT_CODE ,PO_ORD_MAT .MAT_NAME ,Outsource_F_ITEM .ITEM_AU  FROM PO_ORD_MAT JOIN Outsource_F_ITEM ON PO_ORD_MAT .MAT_CODE  =Outsource_F_ITEM .ITEM_CODE WHERE PO_ORD_MAT .MAT_SLNO =" & mat_sl_noDropDownList.Text.Substring(0, mat_sl_noDropDownList.Text.IndexOf(",") - 1) & "and po_ord_mat.po_no='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "'"
        mc1.Connection = conn
        dr = mc1.ExecuteReader
        If dr.HasRows Then
            dr.Read()
            matnameTextBox.Text = dr.Item("MAT_NAME")
            au_TextBox.Text = dr.Item("ITEM_AU")
            dr.Close()
        Else
            conn.Close()
            Return
    ...
    Protected Sub crr_add_Button_Click(sender As Object, e As EventArgs) Handles crr_add_Button.Click
        ...
        ElseIf chalan_qty_TextBox.Text <= 0 Then
            chalan_qty_TextBox.Focus()
            Return
        ...
        ElseIf rcv_qty_TextBox.Text <= 0 Then
            rcv_qty_TextBox.Focus()
            Return
        ...
        ElseIf DropDownList21.SelectedValue = "Select" Then
            DropDownList21.Focus()
            Return
        ElseIf test_location_DropDownList.SelectedValue = "Select" Then
            test_location_DropDownList.Focus()
            Label467.Visible = True
            Label467.Text = "Select test lab test location."
            Return
        End If
        ...
        If CDec(no_of_bagTextBox.Text) > 0 And CDec(bag_weightTextBox.Text) = 0 Then
            bag_weightTextBox.Focus()
            bag_weightTextBox.Text = ""
            Label467.Visible = True
            Label467.Text = "Please enter bag weight."
            Return
        ElseIf CDec(bag_weightTextBox.Text) > 0 And CDec(no_of_bagTextBox.Text) = 0 Then
            no_of_bagTextBox.Focus()
            no_of_bagTextBox.Text = ""
            Label467.Visible = True
            Label467.Text = "Please enter number of bags."
            Return
        End If
        ...
        Dim mc1 As New SqlCommand
        conn.Open()
        mc1.CommandText = "select SO_ACTUAL_DATE,PO_TYPE from ORDER_DETAILS where so_no='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "'"
        mc1.Connection = conn
        dr = mc1.ExecuteReader
        If dr.HasRows Then
            dr.Read()
        ...
        conn.Close()
        ''be search
        If po_type = "OUTSOURCED ITEMS(IMP)" Then
            conn.Open()
            Dim dt1 As New DataTable()
            da = New SqlDataAdapter("SELECT PO_ORD_MAT .PO_NO FROM BE_DETAILS JOIN PO_ORD_MAT  ON BE_DETAILS .PO_NO =PO_ORD_MAT.PO_NO AND BE_DETAILS .MAT_SLNO =PO_ORD_MAT .MAT_SLNO  WHERE BE_DETAILS .BE_NO ='" & be_TextBox.Text & "' AND PO_ORD_MAT .PO_NO ='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "' AND BE_DETAILS .BE_STATUS ='PENDING' AND PO_ORD_MAT .MAT_SLNO =" & mat_sl_noDropDownList.Text.Substring(0, mat_sl_noDropDownList.Text.IndexOf(",") - 1), conn)
            count = da.Fill(dt1)
            conn.Close()
            If count = 0 Then
        ...
        If podate > CHLN_DATE Then
            ERRLabel.Visible = True
            ERRLabel.Text = "Chalan date cannot be before PO date"
            Return
        Else
        ...
        pf_type = ""
        freight_type = ""
        Dim MAT_QTY_CONSIDERED_FOR_PAYMENT, qty, rqty, sqty, exqty, tolerance, tole_value As Decimal
        tolerance = 0
        ...
        mc2.CommandText = "SELECT SUM(MAT_FREIGHT_PU) AS MAT_FREIGHT_PU, MAX(FREIGHT_TYPE) AS FREIGHT_TYPE,MAX(DISC_TYPE) AS DISC_TYPE, MAX(PF_TYPE) AS PF_TYPE, MAX(MAT_CODE) as mat_code, MAX(AMD_NO) as AMD_NO,MAX(AMD_NO) as AMD_NO,SUM(MAT_QTY) AS MAT_QTY, SUM(MAT_UNIT_RATE ) AS MAT_UNIT_RATE,SUM(MAT_PACK) AS MAT_PACK,SUM(MAT_DISCOUNT) AS MAT_DISCOUNT,SUM(MAT_QTY_RCVD) AS MAT_QTY_RCVD FROM PO_ORD_MAT WHERE PO_NO='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "' AND MAT_SLNO=" & mat_sl_noDropDownList.Text.Substring(0, mat_sl_noDropDownList.Text.IndexOf(",") - 1) & " and AMD_DATE < ='" & CHLN_DATE.Year & "-" & CHLN_DATE.Month & "-" & CHLN_DATE.Day & "'"
        mc2.Connection = conn
        ...
        conn.Open()
        mc1.CommandText = "select SUM(MAT_QTY) AS MAT_QTY , SUM(MAT_QTY_RCVD) AS MAT_QTY_RCVD FROM PO_ORD_MAT WHERE mat_slno=" & mat_sl_noDropDownList.Text.Substring(0, mat_sl_noDropDownList.Text.IndexOf(",") - 1) & " and po_no='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "'"
        mc1.Connection = conn
        dr = mc1.ExecuteReader
        If dr.HasRows Then
            dr.Read()

            If (IsDBNull(dr.Item("MAT_QTY"))) Then
                qty = 0.00
            Else
                qty = dr.Item("MAT_QTY")
            End If
        ...
        conn.Open()
        If (au_TextBox.Text = "PCS") Then
            mc1.CommandText = "select (sum(MAT_RCD_QTY) - sum(MAT_REJ_QTY) - sum(MAT_EXCE)) As MAT_QTY_RCVD,(sum(MAT_RCD_QTY) - sum(MAT_REJ_QTY) - sum(MAT_EXCE)) as MAT_QTY_CONSIDERED_FOR_PAYMENT from PO_RCD_MAT where po_no='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "' and MAT_SLNO='" & mat_sl_noDropDownList.Text.Substring(0, mat_sl_noDropDownList.Text.IndexOf(",") - 1) & "'"
        Else
            mc1.CommandText = "select (sum(MAT_RCD_QTY) - sum(MAT_REJ_QTY) - sum(MAT_EXCE)) As MAT_QTY_RCVD,sum((CASE WHEN (MAT_CHALAN_QTY-MAT_RCD_QTY)<(MAT_CHALAN_QTY*0.005) THEN MAT_CHALAN_QTY ELSE MAT_RCD_QTY END))-sum(MAT_REJ_QTY)-sum(MAT_EXCE) as MAT_QTY_CONSIDERED_FOR_PAYMENT from PO_RCD_MAT where po_no='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "' and MAT_SLNO='" & mat_sl_noDropDownList.Text.Substring(0, mat_sl_noDropDownList.Text.IndexOf(",") - 1) & "'"
        End If
        ...
        Dim w_tolerance, TRANSPORTER_UNIT_PRICE, TRANSPORTER_TOTAL_PRICE, w_unit_price, W_discount, DISCOUNTED_UNIT_PRICE, qty_considered_current_crr As New Decimal(0.00)
        ...
        If trans_DropDownList.SelectedValue <> "N/A" Then
            ...
            Work_completed = 0
            conn.Open()
            mc1.CommandText = "select sum(W_QTY) As WO_Qty, sum(W_COMPLITED) As Work_completed from WO_ORDER where PO_NO='" & trans_DropDownList.Text.Substring(0, trans_DropDownList.Text.IndexOf(",") - 1).Trim & "' and W_SLNO='" & DropDownList21.Text.Substring(0, DropDownList21.Text.IndexOf(",") - 1).Trim & "' group by PO_NO,W_SLNO"
            mc1.Connection = conn
            dr = mc1.ExecuteReader
            If dr.HasRows Then
                dr.Read()
            ...
            If WO_Qty < Work_completed + CDec(TransChalanQty.Text) Then
                chalan_qty_TextBox.Focus()
                Label467.Visible = True
                Label467.Text = "Chalan Qty is Higher Than Transporter Balance Qty which is. " & WO_Qty - Work_completed
                Return
            Else
                Label467.Text = ""
                Label467.Visible = False
        ...
        TRANSPORTER_UNIT_PRICE = (w_unit_price - ((w_unit_price * W_discount) / 100))


        If (au_TextBox.Text = "PCS") Then
            qty_considered_current_crr = CDec(rcv_qty_TextBox.Text)
            TRANSPORTER_TOTAL_PRICE = TRANSPORTER_UNIT_PRICE * CDec(TransChalanQty.Text)
        Else
            If ((CDec(chalan_qty_TextBox.Text) - CDec(bag_weightTextBox.Text)) * 0.995) < net_rcd_qty.Text Then
                qty_considered_current_crr = CDec(chalan_qty_TextBox.Text) - CDec(bag_weightTextBox.Text)
                TRANSPORTER_TOTAL_PRICE = TRANSPORTER_UNIT_PRICE * CDec(chalan_qty_TextBox.Text)
            Else
                qty_considered_current_crr = CDec(rcv_qty_TextBox.Text) - CDec(bag_weightTextBox.Text)
                TRANSPORTER_TOTAL_PRICE = TRANSPORTER_UNIT_PRICE * CDec(net_rcd_qty.Text)
            End If
        ...
        If (au_TextBox.Text = "PCS") Then
            If (qty - MAT_QTY_CONSIDERED_FOR_PAYMENT) < CDec(rcv_qty_TextBox.Text) Then
                exqty = (CDec(rcv_qty_TextBox.Text)) - (qty - MAT_QTY_CONSIDERED_FOR_PAYMENT)
            Else
                exqty = 0.0
            End If
        Else
            If (qty - MAT_QTY_CONSIDERED_FOR_PAYMENT) < ((CDec(chalan_qty_TextBox.Text) - CDec(bag_weightTextBox.Text))) Then
                exqty = ((CDec(chalan_qty_TextBox.Text) - CDec(bag_weightTextBox.Text))) - (qty - MAT_QTY_CONSIDERED_FOR_PAYMENT)
            Else
                exqty = 0.0
            End If
        End If
        ...
        If exqty >= CDec(net_rcd_qty.Text) Then
            net_rcd_qty.Text = ""
            net_rcd_qty.Focus()
            Return
        End If
        ...
        conn.Open()
        mc1.CommandText = "select FREIGHT_TERM from ORDER_DETAILS where SO_NO='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "'"
        mc1.Connection = conn
        dr = mc1.ExecuteReader
        If dr.HasRows Then
            dr.Read()
        ...
        'Dim dt As DataTable = DirectCast(ViewState("mat"), DataTable)
        dt.Clear()
        dt.Rows.Add(mat_sl_noDropDownList.Text.Substring(0, mat_sl_noDropDownList.Text.IndexOf(",") - 1), mat_id, matnameTextBox.Text, au_TextBox.Text, crr_TextBox.Text, chalan_TextBox.Text, chalandate_TextBox.Text, be_TextBox.Text, bedate_TextBox.Text, bl_noTextBox.Text, bldate_TextBox.Text, qty, chalan_qty_TextBox.Text, rcv_qty_TextBox.Text, CDec(CDec(chalan_qty_TextBox.Text) - CDec(rcv_qty_TextBox.Text)), exqty, sqty, NO_OF_BAG, Math.Round(BAG_WEIGHT, 3), Math.Round(TRANSPORTER_TOTAL_PRICE, 3), tole_value, AMD_NO, TransRcdQty.Text, test_location_DropDownList.SelectedValue, TransChalanQty.Text, TransRcdQty.Text)
        ViewState("mat") = dt
        Me.BINDGRID1()
        crr_TextBox.Text = ""
    End Sub
    ...
    Protected Sub crr_save_Button_Click(sender As Object, e As EventArgs) Handles crr_save_Button.Click
        ...
        If crr_gridview.Rows.Count = 0 Then
            Return
        ElseIf (pono_DropDownList.SelectedValue = "Select") Then
            Return
        End If
        Dim working_date As Date
        If TextBox2.Text = "" Then
        ...
        Using conn_trans
            ...
            Try
                ...
                If (CDate(TextBox2.Text) <= CDate(Block_DATE)) Then
                    ...
                    Label22.Text = "CRR before " & Block_DATE & " has been freezed."
                    crr_save_Button.Enabled = False
                Else
                    ...
                    Dim mat_tolerance, WEIGHT_DIFF, mat_tolerance_value As New Decimal(0.00)
                    conn.Open()
                    mc1.CommandText = "SELECT PARTY_CODE,TOLERANCE from ORDER_DETAILS WHERE SO_NO='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "'"
                    mc1.Connection = conn
                    dr = mc1.ExecuteReader
                    If dr.HasRows Then
                        dr.Read()
                        ...
                        If (IsDBNull(dr.Item("TOLERANCE"))) Then
                            mat_tolerance = 0.00
                        Else
                            mat_tolerance = dr.Item("TOLERANCE")
                        End If
                    ...
                    Dim TRANS_WO_NO As String = "N/A"
                    Dim TRANS_SLNO As String = "N/A"
                    ...
                    For I = 0 To crr_gridview.Rows.Count - 1
                        ...
                        MAT_CODE = crr_gridview.Rows(I).Cells(1).Text
                        ...
                        Dim TransChlnQty As Decimal = crr_gridview.Rows(I).Cells(24).Text
                        Dim TransRcdQty As String = crr_gridview.Rows(I).Cells(25).Text
                        ...
                        cmd.Parameters.AddWithValue("@INSP_LOCATION", test_location)
                        If (test_location = "SRU") Then
                            cmd.Parameters.AddWithValue("@SAMPLING_STATUS", "PENDING")
                        ...
                        '''''''''''''''''''''''''''''''''
                        Dim C_QTY, R_QTY, N_BAG, B_WEIGHT, FAULT_PRICE, WEIGHT_TO_BE_CONSIDERED As Decimal
                        C_QTY = 0.0
                        R_QTY = 0.0
                        N_BAG = 0.0
                        FAULT_PRICE = 0.0
                        B_WEIGHT = 0.0
                        WEIGHT_TO_BE_CONSIDERED = 0.0
                        For I1 = 0 To crr_gridview.Rows.Count - 1
                            C_QTY = C_QTY + crr_gridview.Rows(I1).Cells(12).Text
                            R_QTY = R_QTY + crr_gridview.Rows(I1).Cells(13).Text
                            N_BAG = N_BAG + crr_gridview.Rows(I1).Cells(17).Text
                            B_WEIGHT = B_WEIGHT + crr_gridview.Rows(I1).Cells(18).Text
                        Next

                        If (au_TextBox.Text = "PCS") Then
                            mat_tolerance_value = 0.00
                            WEIGHT_DIFF = 0.00
                        Else
                            mat_tolerance_value = (CDec(crr_gridview.Rows(0).Cells(12).Text) * mat_tolerance) / 100
                            WEIGHT_DIFF = CDec(crr_gridview.Rows(0).Cells(12).Text) - CDec(crr_gridview.Rows(0).Cells(13).Text)
                        End If
                        ...
                        If trans_DropDownList.Text = "N/A" Then
                            If mat_tolerance_value >= WEIGHT_DIFF Then
                                WEIGHT_TO_BE_CONSIDERED = TransChlnQty
                            Else
                                WEIGHT_TO_BE_CONSIDERED = TransRcdQty

                            End If

                            RCVD_QTY_FOR = WEIGHT_TO_BE_CONSIDERED - CDec(crr_gridview.Rows(I).Cells(15).Text) - ((CDec(crr_gridview.Rows(I).Cells(17).Text) * CDec(crr_gridview.Rows(I).Cells(18).Text)) / 1000)
                        Else
                            RCVD_QTY_FOR = CDec(crr_gridview.Rows(I).Cells(12).Text) - CDec(crr_gridview.Rows(I).Cells(15).Text) - ((CDec(crr_gridview.Rows(I).Cells(17).Text) * CDec(crr_gridview.Rows(I).Cells(18).Text)) / 1000)
                        End If

                        'conn.Open()
                        mycommand = New SqlCommand("update PO_ORD_MAT set MAT_QTY_RCVD=MAT_QTY_RCVD + " & RCVD_QTY_FOR & ",MAT_STATUS='" & str & "'  WHERE MAT_SLNO='" & crr_gridview.Rows(I).Cells(0).Text & "' and PO_NO='" & po_no & "' AND AMD_NO='" & crr_gridview.Rows(I).Cells(21).Text & "'", conn_trans, myTrans)
                        mycommand.ExecuteNonQuery()
                        'conn.Close()
                    Next

                    ''============================TRANSPORTOR CALCULATION================================''
                    ''UPDATE TRANSPORTER
                    If trans_DropDownList.SelectedValue <> "N/A" Then
                        Dim Trans_C_QTY, Trans_R_QTY, N_BAG, B_WEIGHT, FAULT_PRICE, WEIGHT_TO_BE_CONSIDERED As Decimal
                        Trans_C_QTY = 0.0
                        Trans_R_QTY = 0.0
                        N_BAG = 0.0
                        FAULT_PRICE = 0.0
                        ...
                        WEIGHT_TO_BE_CONSIDERED = 0.0
                        For I = 0 To crr_gridview.Rows.Count - 1
                            Trans_C_QTY = Trans_C_QTY + crr_gridview.Rows(I).Cells(24).Text
                            Trans_R_QTY = Trans_R_QTY + crr_gridview.Rows(I).Cells(25).Text
                            N_BAG = N_BAG + crr_gridview.Rows(I).Cells(17).Text
                            B_WEIGHT = B_WEIGHT + crr_gridview.Rows(I).Cells(18).Text
                        Next
                        conn.Open()
                        Dim CHLN_DATE As Date = chalandate_TextBox.Text
                        Dim w_qty, w_complite, w_tolerance, PRICE, w_unit_price, discount, DISCOUNT_VALUE, mat_price As Decimal
                        Dim MC As New SqlCommand
                        Dim w_name, w_au As String
                        ...
                        End If
                        conn.Close()
                        PRICE = Trans_C_QTY * w_unit_price
                        DISCOUNT_VALUE = (PRICE * discount) / 100

                        PRICE = (PRICE - discount)
                        Dim weight As Decimal
                        weight = N_BAG * B_WEIGHT
                        ''TOTAL_QTY = Trans_R_QTY + weight + w_complite




                        Dim TOLE_VALUE, TRANS_DIFF As New Decimal(0)
                        ...
                        If TOLE_VALUE >= TRANS_DIFF Then
                            WEIGHT_TO_BE_CONSIDERED = Trans_C_QTY
                        Else
                            WEIGHT_TO_BE_CONSIDERED = Trans_R_QTY

                        End If

                        'conn.Open()
                        mycommand = New SqlCommand("update WO_ORDER set W_COMPLITED = W_COMPLITED + " & WEIGHT_TO_BE_CONSIDERED & " where PO_NO='" & trans_DropDownList.Text.Substring(0, trans_DropDownList.Text.IndexOf(",") - 1) & "' AND W_SLNO='" & DropDownList21.Text.Substring(0, DropDownList21.Text.IndexOf(",") - 1) & "'  and AMD_DATE =(select max(AMD_DATE) from WO_ORDER where PO_NO ='" & trans_DropDownList.Text.Substring(0, trans_DropDownList.Text.IndexOf(",") - 1) & "' and W_SLNO = '" & DropDownList21.Text.Substring(0, DropDownList21.Text.IndexOf(",") - 1) & "' and AMD_DATE < ='" & CHLN_DATE.Year & "-" & CHLN_DATE.Month & "-" & CHLN_DATE.Day & "')", conn_trans, myTrans)
                        mycommand.ExecuteNonQuery()
                        'conn.Close()
                        ...
                        For count = 0 To crr_gridview.Rows.Count - 1
                            ...
                            prov_price_for_transporter = prov_price_for_transporter + CDec(w_unit_price * WEIGHT_TO_BE_CONSIDERED)
                            chln_qty_trans = chln_qty_trans + Trans_C_QTY
                            rcvd_qty_transporter = rcvd_qty_transporter + Trans_R_QTY
                        ...
                        cmd.Parameters.AddWithValue("@AC_NO", "61997")
                        ...
                        cmd.Parameters.AddWithValue("@work_qty", CDec(WEIGHT_TO_BE_CONSIDERED))
                        cmd.Parameters.AddWithValue("@rqd_qty", CDec(chln_qty_trans))
                        cmd.Parameters.AddWithValue("@bal_qty", CDec(w_qty) - CDec(w_complite))
                    ...
                    If (be_TextBox.Text <> "N/A") Then
                        ...
                        If (CHA_QTY > 0) Then
                            'conn.Open()
                            mycommand = New SqlCommand("update BE_DETAILS set CHA_RCD_QTY=CHA_RCD_QTY + " & CHA_QTY & "  WHERE MAT_SLNO='" & crr_gridview.Rows(0).Cells(0).Text & "' and PO_NO='" & pono_DropDownList.Text.Substring(0, pono_DropDownList.Text.IndexOf(",") - 1) & "' AND BE_NO='" & be_TextBox.Text & "'", conn_trans, myTrans)
                            mycommand.ExecuteNonQuery()
                            'conn.Close()
                        End If
                        ...
                        If CHA_PROV_VALUE > 0 Then
                            ...
                            ''LEDGER POSTING PURCHASE
                            Dim PURCHASE_HEAD As String = "61997"
                    ...
                    Dim dt As New DataTable()
                    dt.Columns.AddRange(New DataColumn(23) {New DataColumn("SlNo"), New DataColumn("Mat Code"), New DataColumn("Mat Name"), New DataColumn("A/U"), New DataColumn("CRR No"), New DataColumn("Chalan No"), New DataColumn("Chalan Date"), New DataColumn("BE No"), New DataColumn("BE Date"), New DataColumn("BL No"), New DataColumn("BL Date"), New DataColumn("Order Qty"), New DataColumn("Chalan Qty"), New DataColumn("Rcvd Qty"), New DataColumn("Short Qty"), New DataColumn("Exces Qty"), New DataColumn("Bal Qty"), New DataColumn("No Of Bag"), New DataColumn("Bag Weight"), New DataColumn("Trans Charge"), New DataColumn("Short Price"), New DataColumn("Amd No"), New DataColumn("Total Mt."), New DataColumn("Lab test location")})
                    ViewState("mat") = dt
                    Me.BINDGRID1()
                    conn.Open()
                    Dim ds5 As New DataSet
                    da = New SqlDataAdapter("select distinct (PO_ORD_MAT.PO_NO +' , '+ ORDER_DETAILS .SO_ACTUAL) AS PO_NO from PO_ORD_MAT WITH(NOLOCK) JOIN ORDER_DETAILS ON PO_ORD_MAT.PO_NO=ORDER_DETAILS.SO_NO where PO_ORD_MAT.MAT_STATUS='PENDING' AND (ORDER_DETAILS.SO_STATUS='ACTIVE' OR ORDER_DETAILS.SO_STATUS='RC') and ORDER_DETAILS.PO_TYPE='" & type_DropDown.SelectedValue & "' ORDER BY PO_NO", conn)
                    da.Fill(ds5, "PO_ORD_MAT")
                    conn.Close()
                    pono_DropDownList.DataSource = ds5.Tables("PO_ORD_MAT")
                    pono_DropDownList.DataValueField = "PO_NO"
                    ...
                    myTrans.Commit()
                    Label467.Visible = True
                    Label467.Text = "All records are written to database."
                End If
 ...
 "MAX(PO_RCD_MAT .INSP_EMP ) AS INSP_EMP , MAX (PO_RCD_MAT .INSP_NOTE ) AS INSP_NOTE , MAX (PO_RCD_MAT .MAT_RATE) AS MAT_RATE , MAX (PO_RCD_MAT .GARN_NOTE ) AS GARN_NOTE , MAX (Outsource_F_ITEM .ITEM_NAME ) AS MAT_NAME , MAX (Outsource_F_ITEM.ITEM_AU ) AS MAT_AU , MAX (SUPL.SUPL_NAME ) AS SUPL_NAME , MAX (SUPL.SUPL_AT ) AS SUPL_AT , " &
 ...
 "from PO_RCD_MAT join ORDER_DETAILS on PO_RCD_MAT .PO_NO =ORDER_DETAILS .so_no join Outsource_F_ITEM on PO_RCD_MAT .MAT_CODE =Outsource_F_ITEM.ITEM_CODE join SUPL on PO_RCD_MAT .SUPL_ID=SUPL.SUPL_ID join PO_ORD_MAT on PO_RCD_MAT .PO_NO =PO_ORD_MAT .PO_NO and PO_RCD_MAT .MAT_SLNO =PO_ORD_MAT .MAT_SLNO  " &
 " where PO_RCD_MAT .CRR_NO ='" & search_DropDown.SelectedValue & "'" &
 " GROUP BY PO_RCD_MAT .MAT_SLNO,PO_RCD_MAT . TRANS_SLNO " &
 " ORDER BY PO_RCD_MAT.MAT_SLNO"
    ...
    Protected Sub DropDownList1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DropDownList1.SelectedIndexChanged
        ...
        If (DropDownList1.SelectedValue <> "Select") Then
            conn.Open()
            da = New SqlDataAdapter("select distinct CRR_NO from PO_RCD_MAT where CRR_NO like 'OCRR%' and crr_date between '" & 2000 + CInt(Left(DropDownList1.SelectedValue, 2)) & "-04-01' and '" & 2000 + CInt(Right(DropDownList1.SelectedValue, 2)) & "-03-31' ORDER BY CRR_NO", conn)
    ...
    '    conn.Open()
    '    dt.Clear()
    '    da = New SqlDataAdapter("SELECT * FROM CHA_CORREC", conn)
    '    da.Fill(dt)
    '    conn.Close()

    '    For Each drRow As DataRow In dt.Rows
    '        'drRow.Item("ColumnName").ToString
    '        Dim CHA_WO, CHA_SLNO, BE_NO_NEW As String
    '        Dim CHA_HEAD As New String("")
    '        Dim BE_QTY, CHA_RCD_QTY, CHA_QTY, CHLN_QTY As New Decimal(0)
    '        CHA_WO = ""
    '        CHA_SLNO = ""
    '        BE_NO_NEW = ""

    '        conn.Open()
    '        mycommand.CommandText = "select CHA_ORDER,CHA_SLNO, BE_NO, BE_QTY, CHA_RCD_QTY from BE_DETAILS where BE_NO in (select BE_NO from po_rcd_mat where CRR_NO='" & drRow.Item("BE_NO").ToString() & "')"
    ...
    '        ''CALCULATING CHA QTY
    '        CHA_QTY = drRow.Item("CHLN").ToString()
    '        CHLN_QTY = drRow.Item("CHLN").ToString()
    ...
    '        'UPDATING BE DETAILS
    '        conn.Open()
    '        mycommand = New SqlCommand("UPDATE BE_DETAILS SET CHA_RCD_QTY=CHA_RCD_QTY+" & CHLN_QTY & " WHERE BE_NO='" & BE_NO_NEW & "'", conn)
    '        mycommand.ExecuteNonQuery()
    '        conn.Close()
    ...
    '        CHA_PROV_VALUE = 0
    '        conn.Open()
    '        MC2.CommandText = "select sum(W_UNIT_PRICE) as W_UNIT_PRICE,sum(W_DISCOUNT) as W_DISCOUNT,max(W_NAME) as W_NAME,max(W_AU) as W_AU from WO_ORDER where PO_NO = '" & CHA_WO & "' AND W_SLNO='" & CHA_SLNO & "' and AMD_DATE < =(SELECT DISTINCT CHLN_DATE FROM PO_RCD_MAT WHERE CRR_NO='" & drRow.Item("BE_NO").ToString() & "')"
    '        MC2.Connection = conn
    '        dr = MC2.ExecuteReader
    '        If dr.HasRows Then
    '            dr.Read()
    ...
    '            conn.Open()
    '            mycommand.CommandText = "select * from work_group where work_name = (SELECT PO_TYPE FROM ORDER_DETAILS WHERE SO_NO='" & CHA_WO & "') and work_type=(select distinct wo_type from wo_order where po_no='" & CHA_WO & "' and w_slno='" & CHA_SLNO & "')"
    '            mycommand.Connection = conn
    '            dr = mycommand.ExecuteReader
    '            If dr.HasRows Then
    '                dr.Read()
    ...
    '            conn.Open()
    '            Dim MCc As New SqlCommand
    '            MCc.CommandText = "select AC_PUR,AC_CON from MATERIAL where MAT_CODE IN (SELECT MAT_CODE FROM PO_RCD_MAT WHERE CRR_NO ='" & drRow.Item("BE_NO").ToString() & "')"
    '            MCc.Connection = conn
    '            dr = MCc.ExecuteReader
    '            If dr.HasRows Then
    '                dr.Read()
    ...
    '            conn.Open()
    '            MCc.CommandText = "select CRR_DATE from PO_RCD_MAT where CRR_NO='" & drRow.Item("BE_NO").ToString() & "'"
    '            MCc.Connection = conn
    '            dr = MCc.ExecuteReader
    '            If dr.HasRows Then
    '                dr.Read()
    ...
    '            cmd1.Parameters.AddWithValue("@PO_NO", CHA_WO)
    '            cmd1.Parameters.AddWithValue("@Journal_ID", CHA_SLNO)
    '            cmd1.Parameters.AddWithValue("@GARN_NO_MB_NO", "CHA" & drRow.Item("BE_NO").ToString())
    '            cmd1.Parameters.AddWithValue("@SUPL_ID", CHA_SUPL_ID)
    '            cmd1.Parameters.AddWithValue("@FISCAL_YEAR", STR1)
    '            cmd1.Parameters.AddWithValue("@PERIOD", qtr1)
    '            cmd1.Parameters.AddWithValue("@EFECTIVE_DATE", working_date)
    '            cmd1.Parameters.AddWithValue("@ENTRY_DATE", Now)
    '            cmd1.Parameters.AddWithValue("@AC_NO", PURCHASE_HEAD)
    '            cmd1.Parameters.AddWithValue("@AMOUNT_DR", CHA_PROV_VALUE)
    '            cmd1.Parameters.AddWithValue("@AMOUNT_CR", 0)
    '            cmd1.Parameters.AddWithValue("@POST_INDICATION", "MATERIAL PURCHASE")
    '            cmd1.Parameters.AddWithValue("@PAYMENT_INDICATION", "")
    '            cmd1.Parameters.AddWithValue("@JURNAL_LINE_NO", 1)
    '            cmd1.Parameters.AddWithValue("@BE_NO", BE_NO_NEW)
    '            cmd1.ExecuteReader()
    '            cmd1.Dispose()
    '            conn.Close()
    ...
    '            ''Ledger CREDIT entry
    '            conn.Open()
    '            Dim QUARY2 As String
    '            QUARY2 = "Insert Into LEDGER(BE_NO,Journal_ID,JURNAL_LINE_NO,PO_NO,GARN_NO_MB_NO,SUPL_ID,FISCAL_YEAR,PERIOD,EFECTIVE_DATE,ENTRY_DATE,AC_NO,AMOUNT_DR,AMOUNT_CR,POST_INDICATION,PAYMENT_INDICATION)VALUES(@BE_NO,@Journal_ID,@JURNAL_LINE_NO,@PO_NO,@GARN_NO_MB_NO,@SUPL_ID,@FISCAL_YEAR,@PERIOD,@EFECTIVE_DATE,@ENTRY_DATE,@AC_NO,@AMOUNT_DR,@AMOUNT_CR,@POST_INDICATION,@PAYMENT_INDICATION)"
    '            Dim cmd2 As New SqlCommand(QUARY1, conn)
    '            cmd2.Parameters.AddWithValue("@PO_NO", CHA_WO)
    '            cmd2.Parameters.AddWithValue("@Journal_ID", CHA_SLNO)
    '            cmd2.Parameters.AddWithValue("@GARN_NO_MB_NO", "CHA" & drRow.Item("BE_NO").ToString())
    '            cmd2.Parameters.AddWithValue("@SUPL_ID", CHA_SUPL_ID)
    '            cmd2.Parameters.AddWithValue("@FISCAL_YEAR", STR1)
    '            cmd2.Parameters.AddWithValue("@PERIOD", qtr1)
    '            cmd2.Parameters.AddWithValue("@EFECTIVE_DATE", working_date)
    '            cmd2.Parameters.AddWithValue("@ENTRY_DATE", Now)
    '            cmd2.Parameters.AddWithValue("@AC_NO", CHA_HEAD)
    '            cmd2.Parameters.AddWithValue("@AMOUNT_DR", 0)
    '            cmd2.Parameters.AddWithValue("@AMOUNT_CR", CHA_PROV_VALUE)
    '            cmd2.Parameters.AddWithValue("@POST_INDICATION", "PROV. FOR CHA")
    '            cmd2.Parameters.AddWithValue("@PAYMENT_INDICATION", "")
    '            cmd2.Parameters.AddWithValue("@JURNAL_LINE_NO", 2)
    '            cmd2.Parameters.AddWithValue("@BE_NO", BE_NO_NEW)
    '            cmd2.ExecuteReader()
    '            cmd2.Dispose()
    '            conn.Close()
    ...
    '            MC2.CommandText = "select MAX(WO_AMD) AS WO_AMD ,MAX(AMD_DATE) AS AMD_DATE, MAX(SUPL_ID) as SUPL_ID,sum(W_MATERIAL_COST) as W_MATERIAL_COST, sum(W_QTY) as W_QTY,sum(W_COMPLITED) as W_COMPLITED,sum(W_TOLERANCE) as W_TOLERANCE,sum(W_UNIT_PRICE) as W_UNIT_PRICE,sum(W_DISCOUNT) as W_DISCOUNT,max(W_NAME) as W_NAME,max(W_AU) as W_AU from WO_ORDER where PO_NO = '" & CHA_WO & "' AND W_SLNO='" & CHA_SLNO & "' and AMD_DATE < =(SELECT DISTINCT CHLN_DATE FROM PO_RCD_MAT WHERE CRR_NO='" & drRow.Item("BE_NO").ToString() & "')"
    '            MC2.Connection = conn
    '            dr = MC2.ExecuteReader
    '            If dr.HasRows Then
    '                dr.Read()
    ...
    '            conn.Open()
    '            Dim Query As String = "Insert Into mb_book(mb_no,mb_date,po_no,supl_id,wo_slno,w_name,w_au,from_date,to_date,work_qty,rqd_qty,bal_qty,note,mb_by,ra_no,prov_amt,pen_amt,sgst,cgst,igst,cess,sgst_liab,cgst_liab,igst_liab,cess_liab,it_amt,pay_ind,fiscal_year,mat_rate,mb_clear,amd_no,amd_date)VALUES(@mb_no,@mb_date,@po_no,@supl_id,@wo_slno,@w_name,@w_au,@from_date,@to_date,@work_qty,@rqd_qty,@bal_qty,@note,@mb_by,@ra_no,@prov_amt,@pen_amt,@sgst,@cgst,@igst,@cess,@sgst_liab,@cgst_liab,@igst_liab,@cess_liab,@it_amt,@pay_ind,@fiscal_year,@mat_rate,@mb_clear,@amd_no,@amd_date)"
    '            Dim cmd As New SqlCommand(Query, conn)
    '            cmd.Parameters.AddWithValue("@mb_no", "CHA" & drRow.Item("BE_NO").ToString())
    '            cmd.Parameters.AddWithValue("@mb_date", Date.ParseExact(CDate(CRR_DATE), "dd-MM-yyyy", provider))
    '            cmd.Parameters.AddWithValue("@po_no", CHA_WO)
    '            cmd.Parameters.AddWithValue("@supl_id", WO_SUPL_ID)
    '            cmd.Parameters.AddWithValue("@wo_slno", CHA_SLNO)
    '            cmd.Parameters.AddWithValue("@w_name", WO_NAME)
    '            cmd.Parameters.AddWithValue("@w_au", WO_AU)
    '            cmd.Parameters.AddWithValue("@from_date", Date.ParseExact(CDate(CRR_DATE), "dd-MM-yyyy", provider))
    '            cmd.Parameters.AddWithValue("@to_date", Date.ParseExact(CDate(CRR_DATE), "dd-MM-yyyy", provider))
    '            cmd.Parameters.AddWithValue("@work_qty", CHA_QTY)
    '            cmd.Parameters.AddWithValue("@rqd_qty", CHA_QTY)
    '            cmd.Parameters.AddWithValue("@bal_qty", CDec(w_qty) - (CDec(w_complite) + CHA_QTY))
    '            cmd.Parameters.AddWithValue("@note", "")
    ...
    '            ''update cha work order
    '            conn.Open()
    '            mycommand = New SqlCommand("update WO_ORDER set W_COMPLITED =" & w_complite + CHA_QTY & " where PO_NO='" & CHA_WO & "' AND W_SLNO='" & CHA_SLNO & "'  and AMD_DATE =(select max(AMD_DATE) from WO_ORDER where PO_NO ='" & CHA_WO & "' and W_SLNO = '" & CHA_SLNO & "' and AMD_DATE < =(SELECT DISTINCT CHLN_DATE FROM PO_RCD_MAT WHERE CRR_NO='" & crr_TextBox.Text & "'))", conn)
    '            mycommand.ExecuteNonQuery()
    '            conn.Close()
    '        End If

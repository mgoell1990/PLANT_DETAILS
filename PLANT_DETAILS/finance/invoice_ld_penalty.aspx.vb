...
Public Class invoice_ld_penalty
    ...
    Protected Sub Button35_Click(sender As Object, e As EventArgs) Handles Button35.Click
        ...
        If Panel3.Visible = True Then
            If DropDownList2.SelectedValue = "Tax Invoice" Then
                conn.Open()
                dt.Clear()
                da = New SqlDataAdapter("select m1.mb_no, m1. wo_slno, m1.w_name, m1.w_au,996511 as sac_code, g1.UNIT_PRICE, m1.work_qty, m1.prov_amt, g1.CGST, m1.cgst_liab, g1.SGST, m1.sgst_liab, g1.IGST, m1.igst_liab, g1.CESS, m1.cess_liab,(prov_amt + m1.sgst_liab + m1.cgst_liab + m1.igst_liab + m1.cess_liab) as TOTAL_VAL from PO_RCD_MAT m1 join GARN_PRICE g1 on (m1.mb_no = g1. CRR_NO AND m1.po_no=g1.PO_NO) where m1. po_no='" & TextBox124.Text & "' and v_ind='V' and inv_no='" & DropDownList30.SelectedValue & "' and RCM_P is null", conn)
                da.Fill(dt)
                conn.Close()
                GridView2.DataSource = dt
                GridView2.DataBind()

            ElseIf DropDownList2.SelectedValue = "Payment Voucher" Then

                conn.Open()
                dt.Clear()
                'da = New SqlDataAdapter("select m1.mb_no, m1. wo_slno, m1.w_name, m1.w_au,999794 as sac_code, g1.UNIT_PRICE, m1.work_qty, (m1.pen_amt + m1.ld) as prov_amt, g1.CGST, m1.rcm_cgst as cgst_liab, g1.SGST, m1.rcm_sgst as sgst_liab, g1.IGST, m1.rcm_igst as igst_liab, g1.CESS, m1.rcm_cess as cess_liab, (m1.pen_amt + m1.ld + m1.rcm_cgst + m1.rcm_sgst + m1.rcm_igst + m1.rcm_cess) as TOTAL_VAL from PO_RCD_MAT m1 join GARN_PRICE g1 on (m1.mb_no = g1. CRR_NO AND m1.po_no=g1.PO_NO) where m1. po_no='" & TextBox124.Text & "' and v_ind='V' and inv_no='" & DropDownList30.SelectedValue & "' AND (pen_amt > 0 or ld > 0) and RCM_F is null", conn)
                'da = New SqlDataAdapter("select p1.CRR_NO, p1. MAT_SLNO,m1.MAT_NAME,m1.MAT_AU,999794 as sac_code, g1.UNIT_PRICE, p1.MAT_CHALAN_QTY, p1.MAT_RCD_QTY, (p1.PENALITY_CHARGE + p1.LD_CHARGE) as prov_amt, g1.CGST, p1.rcm_cgst as cgst_liab, g1.SGST, p1.rcm_sgst as sgst_liab, g1.IGST, p1.rcm_igst as igst_liab, g1.CESS, p1.rcm_cess as cess_liab, (p1.PENALITY_CHARGE + p1.LD_CHARGE + p1.rcm_cgst + p1.rcm_sgst + p1.rcm_igst + p1.rcm_cess) as TOTAL_VAL from PO_RCD_MAT p1 join GARN_PRICE g1 on (p1.CRR_NO = g1. CRR_NO AND p1.po_no=g1.PO_NO) join MATERIAL m1 on p1.MAT_CODE=m1.MAT_CODE where P1.po_no='" & TextBox124.Text & "' and v_ind='V' and inv_no='" & DropDownList30.SelectedValue & "' AND (P1.PENALITY_CHARGE > 0 or P1.LD_CHARGE > 0) and RCM_F is null", conn)
                da = New SqlDataAdapter("select p1.CRR_NO, p1. MAT_SLNO, m1.MAT_NAME, m1.MAT_AU,999794 as sac_code, p1.UNIT_RATE AS UNIT_PRICE, p1.MAT_CHALAN_QTY, p1.MAT_RCD_QTY, (p1.PENALITY_CHARGE + p1.LD_CHARGE) as prov_amt, 
                    CAST((round((p1.rcm_cgst*100/(p1.PENALITY_CHARGE + p1.LD_CHARGE)),1)) as decimal(18,3)) AS CGST, p1.rcm_cgst as cgst_liab, CAST((round((p1.rcm_sgst*100/(p1.PENALITY_CHARGE + p1.LD_CHARGE)),1)) as decimal(18,3)) AS SGST, 
                    p1.rcm_sgst as sgst_liab, CAST((round((p1.rcm_igst*100/(p1.PENALITY_CHARGE + p1.LD_CHARGE)),1)) as decimal(18,3)) AS IGST, p1.rcm_igst as igst_liab, CAST((round((p1.rcm_cess*100/(p1.PENALITY_CHARGE + p1.LD_CHARGE)),1)) as decimal(18,3)) AS CESS, 
                    p1.rcm_cess as cess_liab,(p1.PENALITY_CHARGE + p1.LD_CHARGE + p1.rcm_cgst + p1.rcm_sgst + p1.rcm_igst + p1.rcm_cess) as TOTAL_VAL from po_rcd_mat p1 join MATERIAL m1 on (p1.MAT_CODE=m1.MAT_CODE) where p1. po_no='" & TextBox124.Text & "' 
                    and v_ind='V' and inv_no='" & DropDownList30.SelectedValue & "' AND (PENALITY_CHARGE > 0 or LD_CHARGE > 0) and RCM_F is null", conn)
                da.Fill(dt)
                conn.Close()
                GridView2.DataSource = dt
                GridView2.DataBind()
            ...
            Try
                ...
                For N = 0 To GridView2.Rows.Count - 1
                    ...
                    If DropDownList2.SelectedValue = "Payment Voucher" Then
                        ...
                        QUARY1 = "UPDATE PO_RCD_MAT SET RCM_F ='P' WHERE CRR_NO ='" & GridView2.Rows(N).Cells(0).Text & "' AND INV_NO ='" & DropDownList30.SelectedValue & "' AND MAT_SLNO ='" & GridView2.Rows(N).Cells(1).Text & "' AND PO_NO ='" & TextBox124.Text & "'"
                        Dim cmdd As New SqlCommand(QUARY1, conn_trans, myTrans)
                        cmdd.ExecuteReader()
                        cmdd.Dispose()

                    ElseIf DropDownList2.SelectedValue = "Tax Invoice" Then

                        'UPDATE PO_RCD_MAT RCM_F
                        QUARY1 = "UPDATE PO_RCD_MAT SET RCM_P ='P' WHERE mb_no ='" & GridView2.Rows(N).Cells(0).Text & "' AND INV_NO ='" & DropDownList30.SelectedValue & "' AND MAT_SLNO ='" & GridView2.Rows(N).Cells(1).Text & "' AND PO_NO ='" & TextBox124.Text & "'"
                        Dim cmdd As New SqlCommand(QUARY1, conn_trans, myTrans)
                        cmdd.ExecuteReader()
                        cmdd.Dispose()
    ...
    Protected Sub generateRCM(flag As String, type As String, DESPATCH_TYPE As String, inv_type As String, inv_type1 As String, inv_rule As String, inv_for As String, SUPL_INV As String, STR1 As String, COMM As String, DIVISION As String)
        ...
        cmd.Parameters.AddWithValue("@CGST", CDec(GridView2.Rows(0).Cells(8).Text))
        cmd.Parameters.AddWithValue("@CGST_AMT", total_cgst)
        cmd.Parameters.AddWithValue("@SGST", CDec(GridView2.Rows(0).Cells(10).Text))
        cmd.Parameters.AddWithValue("@SGST_AMT", total_sgst)
        cmd.Parameters.AddWithValue("@IGST", CDec(GridView2.Rows(0).Cells(12).Text))
        cmd.Parameters.AddWithValue("@IGST_AMT", total_igst)
        cmd.Parameters.AddWithValue("@CESS", CDec(GridView2.Rows(0).Cells(14).Text))
    ...
    Protected Sub DropDownList2_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DropDownList2.SelectedIndexChanged
        ...
        If DropDownList2.SelectedValue = "Tax Invoice" Then
            ...
            conn.Open()
            dt.Clear()
            da = New SqlDataAdapter("SELECT DISTINCT INV_NO FROM PO_RCD_MAT WHERE po_no='" & TextBox124.Text & "' AND v_ind IS NOT NULL and  RCM_P is null", conn)
            da.Fill(dt)
            conn.Close()
        ...
        ElseIf DropDownList2.SelectedValue = "Payment Voucher" Then
            ...
            conn.Open()
            dt.Clear()
            'da = New SqlDataAdapter("SELECT DISTINCT INV_NO FROM PO_RCD_MAT WHERE  po_no='" & TextBox124.Text & "' AND (pen_amt > 0 or ld > 0) and v_ind='v' and  RCM_F is null", conn)
            da = New SqlDataAdapter("Select DISTINCT INV_NO FROM PO_RCD_MAT WHERE  po_no='" & TextBox124.Text & "' AND (LD_CHARGE > 0 or PENALITY_CHARGE > 0) and v_ind='v' and  RCM_F is null", conn)
            da.Fill(dt)
            conn.Close()

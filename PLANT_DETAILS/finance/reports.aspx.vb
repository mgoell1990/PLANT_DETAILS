...
ORDER BY RowType"
    ...
    Protected Sub Button9_Click(sender As Object, e As EventArgs) Handles Button9.Click
        ...
        If DropDownList2.Text = "All" Then
            ...
            SELECT T1.AC_NO,ac_description,T1.PO_NO,T1.GARN,T1.VOUCHER_NO,T1.EFECTIVE_DATE,T1.DR,T1.CR,(CASE WHEN T1.PO_NO LIKE 'P%' THEN (SELECT top 1 'INCOMING MATERIAL ('+MAT_NAME+')' FROM PO_RCD_MAT WHERE GARN_NO=T1.GARN) WHEN T1.PO_NO LIKE 'S%' THEN (SELECT top 1 'OUTWARD SUPPLY ('+P_DESC+')' FROM DESPATCH WHERE D_TYPE+INV_NO=T1.GARN AND FISCAL_YEAR=" & STR1 & ") ELSE VOUCHER.PARTICULAR END) AS DESCRIPTION FROM @TT T1 LEFT JOIN VOUCHER ON T1.VOUCHER_NO=VOUCHER.TOKEN_NO ORDER BY AC_NO, EFECTIVE_DATE"
            da = New SqlDataAdapter(quary, conn)
            da.Fill(dt)
            conn.Close()
